...
import {
  ...
  submitPromotionDemotionTransferEntriesService,
...
export const getAllPromotionDemotionTransferEntries = async (
  ...
  try {
    ...
    const items = await getAllPromotionDemotionTransferEntriesService(status, allowedCompanyIds, fromDate, toDate);
...
export const submitPromotionDemotionTransferEntry = async (
  req: Request,
  res: Response
): Promise<void> => {
  const { refNo } = req.params;
  ...
  try {
    const result = await submitPromotionDemotionTransferEntriesService(String(refNo), Role as string);
    res.json({ success: true, message: result.message || "Promotion / demotion / transfer entries submitted successfully" });
  } catch (error: any) {
    console.error("SubmitPromotionDemotionTransferEntry error:", error);
    res.status(500).json({ success: false, message: error?.message || "Internal server error" });
  }
};
