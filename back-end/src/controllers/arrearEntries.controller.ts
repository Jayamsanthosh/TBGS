...
export const getAllArrearEntries = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const items = await getAllArrearEntriesService(status, allowedCompanyIds, fromDate, toDate);
...
export const submitArrearEntries = async (req: Request, res: Response): Promise<void> => {
  const { id } = req.params;
  const { Role } = req.body;
  ...
  try {
    const result = await submitArrearEntriesService(String(id), Role as string);
    res.json({ success: true, message: result.message || "Arrear entries submitted successfully" });
  } catch (error: any) {
    console.error("SubmitArrearEntries error:", error);
    res.status(500).json({ success: false, message: error?.message || "Internal server error" });
  }
};
