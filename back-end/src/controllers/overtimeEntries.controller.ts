...
export const getAllOvertimeEntries = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const items = await getAllOvertimeEntriesService(status, allowedCompanyIds, fromDate, toDate);
...
export const submitOvertimeEntries = async (req: Request, res: Response): Promise<void> => {
  const { id } = req.params;
  const { Role } = req.body;
  ...
  try {
    const result = await submitOvertimeEntriesService(String(id), Role as string);
    res.json({ success: true, message: result.message || "Overtime entries submitted successfully" });
  } catch (error: any) {
    console.error("SubmitOvertimeEntries error:", error);
    res.status(500).json({ success: false, message: error?.message || "Internal server error" });
  }
};
