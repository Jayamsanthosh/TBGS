...
export const getAllDeductionEntries = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const entries = await getAllDeductionEntriesService(status, allowedCompanyIds, fromDate, toDate);
...
export const submitDeductionEntries = async (req: Request, res: Response): Promise<void> => {
  const { id } = req.params;
  const { Role } = req.body;
  ...
  try {
    const result = await submitDeductionEntriesService(String(id), Role as string);
    res.json({ success: true, message: result.message || "Deduction entries submitted successfully" });
  } catch (error: any) {
    console.error("SubmitDeductionEntries error:", error);
    res.status(500).json({ success: false, message: error?.message || "Internal server error" });
  }
};
