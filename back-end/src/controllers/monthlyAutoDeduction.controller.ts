...
import {
  ...
  submitMonthlyAutoDeductionService,
...
export const getAllMonthlyAutoDeductions = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const deductions = await getAllMonthlyAutoDeductionsService(status, allowedCompanyIds, fromDate, toDate);
...
export const submitMonthlyAutoDeduction = async (req: Request, res: Response): Promise<void> => {
  const { id } = req.params;
  const { Role } = req.body;
  ...
  try {
    const result = await submitMonthlyAutoDeductionService(parseInt(id as string, 10), Role as string);
    res.json({ success: true, message: result.message || "Monthly auto deduction submitted successfully" });
  } catch (error: any) {
    console.error("SubmitMonthlyAutoDeduction error:", error);
    res.status(500).json({ success: false, message: error?.message || "Internal server error" });
  }
};
