...
import {
  ...
  submitOverTimeReferenceEntriesService,
...
export const getAllOverTimeReferenceEntries = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const items = await getAllOverTimeReferenceEntriesService(status, allowedCompanyIds, fromDate, toDate);
...
export const submitOverTimeReferenceEntries = async (req: Request, res: Response): Promise<void> => {
  const { id } = req.params;
  const { Role } = req.body;
  ...
  try {
    const result = await submitOverTimeReferenceEntriesService(String(id), Role as string);
    res.json({ success: true, message: result.message || "Over time reference entry submitted successfully" });
  } catch (error: any) {
    console.error("SubmitOverTimeReferenceEntries error:", error);
    res.status(400).json({ success: false, message: error?.message || "Internal server error" });
  }
};
