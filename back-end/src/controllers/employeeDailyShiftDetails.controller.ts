...
import {
  ...
  submitEmployeeDailyShiftDetailsService,
...
export const getAllEmployeeDailyShiftDetails = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const records = await getAllEmployeeDailyShiftDetailsService(empId, status, fromDate, toDate);
...
export const submitEmployeeDailyShiftDetails = async (req: Request, res: Response): Promise<void> => {
  const { id } = req.params;
  const { Role } = req.body;
  ...
  try {
    const result = await submitEmployeeDailyShiftDetailsService(parseInt(id as string, 10), Role as string);
    res.json({ success: true, message: result.message || "Employee daily shift detail submitted successfully" });
  } catch (error: any) {
    console.error("SubmitEmployeeDailyShiftDetails error:", error);
    res.status(400).json({ success: false, message: error?.message || "Internal server error" });
  }
};
