...
import {
  getAllFilesService,
  getFilesByDriverService,
  getFileByIdService,
  saveFileService,
  updateFileService,
  deleteFileService,
  DriverMasterFileData
} from "../services/driverMasterFiles.services";
...
const exceedsFileLimit = (data: DriverMasterFileData["CONTENT_DATA"]): boolean => {
...
export const getFilesByDriver = async (req: Request, res: Response): Promise<void> => {
  const driverEmpId = parseId(req.query.driverId as string);
  if (!driverEmpId) {
    res.status(400).json({ success: false, message: "A valid driverId query param is required" });
    return;
  }
  try {
    const status = req.query.status as string | undefined;
    const files = await getFilesByDriverService(driverEmpId, status || undefined);
    res.json({ success: true, count: files.length, data: files });
  } catch (error: any) {
    console.error("GetFilesByDriver error:", error);
    res.status(500).json({ success: false, message: error?.message || "Internal server error" });
  }
};
...
export const getFileById = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    const file = await getFileByIdService(sno, USER, ROLE, MAC_ADDRESS);
    if (!file) {
...
export const saveFile = async (req: Request, res: Response): Promise<void> => {
  const fileData: DriverMasterFileData = req.body;
  if (!parseId(String(fileData.DRIVER_EMP_ID ?? ""))) {
    res.status(400).json({ success: false, message: "A valid DRIVER_EMP_ID is required" });
    return;
  }
