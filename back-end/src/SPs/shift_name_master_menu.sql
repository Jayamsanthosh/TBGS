...
-- Adds the /shift-name-master link under Masters > HR & Payroll and maps it to the Admin role.
...
IF @SUB_MENU_ID IS NOT NULL AND NOT EXISTS (
    SELECT 1 FROM VMaster.TBL_LINKS_AND_PAGES WHERE LINK_LOCATION = '/shift-name-master'
)
BEGIN
    ...
    VALUES
        (@SUB_MENU_ID, 'Shift Name Master', '/shift-name-master', 'internal', '/shift-name-master', @LINK_SEQ, 'Clock', 'AC', 'System', GETDATE());
    ...
    PRINT 'Shift Name Master link added (LINK_ID = ' + CAST(@LINK_ID AS VARCHAR(10)) + ').';
END
ELSE
BEGIN
    PRINT 'Shift Name Master link already exists - no change.';
END
GO
