...
-- Fix: SAVE_ARREAR_ENTRIES referenced @Company_Id in VALUES and in the EXEC
-- call to SP_Generate_Screen_Ref_No_With_Output_Parameter, but the parameter
-- was never declared, so the SP could not run. Added @COMPANY_ID INT param.
...
BEGIN
    ...
    EXEC VRequest.SP_Generate_Screen_Ref_No_With_Output_Parameter 'ARREAR REQUEST REF NO', @COMPANY_ID, @ARREAR_REQUEST_REF_NO OUT

    IF (@ARREAR_REQUEST_REF_NO IS NULL OR @ARREAR_REQUEST_REF_NO='')
    BEGIN
        SELECT 'Error','Can''t Generate Arres Request Reference Number. Contact Admin',''
    END
    ELSE
    BEGIN
