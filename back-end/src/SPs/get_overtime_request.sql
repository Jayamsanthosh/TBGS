-- =============================================================
-- [VRequest].[GET_OVERTIME_REQUEST]  (Dual-mode)
--   * @OT_REQUEST_REF_NO given -> single-record detail (entry screen).
--   * ref blank                 -> Overtime list for the Report Dashboard,
--                                 same aliases/filters/paging as
--                                 GET_REQUEST_LIST_BY_TYPE
--                                 ('Overtime Request').
-- =============================================================
CREATE OR ALTER PROCEDURE [VRequest].[GET_OVERTIME_REQUEST]
(
    @OT_REQUEST_REF_NO VARCHAR(50) = NULL,
...
BEGIN
    ...
    IF LTRIM(RTRIM(ISNULL(@OT_REQUEST_REF_NO, ''))) <> ''
    BEGIN
        SELECT
            SNO,
            OT_REQUEST_REF_NO,
