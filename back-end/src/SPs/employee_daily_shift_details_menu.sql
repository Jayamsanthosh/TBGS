...
-- Adds the /employee-daily-shift-details link under Entries > Payroll Entries and maps it to the Admin role.
...
IF @SUB_MENU_ID IS NOT NULL AND NOT EXISTS (
    SELECT 1 FROM VMaster.TBL_LINKS_AND_PAGES WHERE LINK_LOCATION = '/employee-daily-shift-details'
)
BEGIN
    ...
    VALUES
        (@SUB_MENU_ID, 'Employee Daily Shift Details', '/employee-daily-shift-details', 'internal', '/employee-daily-shift-details', @LINK_SEQ, 'CalendarClock', 'AC', 'System', GETDATE());
    ...
    PRINT 'Employee Daily Shift Details link added (LINK_ID = ' + CAST(@LINK_ID AS VARCHAR(10)) + ').';
END
ELSE
BEGIN
    PRINT 'Employee Daily Shift Details link already exists - no change.';
END
GO
