...
-- Adds the /employee-benefit-master link under Masters > HR & Payroll and maps it to the Admin role.
...
IF @SUB_MENU_ID IS NOT NULL AND NOT EXISTS (
    SELECT 1 FROM VMaster.TBL_LINKS_AND_PAGES WHERE LINK_LOCATION = '/employee-benefit-master'
)
BEGIN
    ...
    VALUES
        (@SUB_MENU_ID, 'Employee Benefit Master', '/employee-benefit-master', 'internal', '/employee-benefit-master', @LINK_SEQ, 'DollarSign', 'AC', 'System', GETDATE());
    ...
    PRINT 'Employee Benefit Master link added (LINK_ID = ' + CAST(@LINK_ID AS VARCHAR(10)) + ').';
END
ELSE
BEGIN
    PRINT 'Employee Benefit Master link already exists - no change.';
END
GO