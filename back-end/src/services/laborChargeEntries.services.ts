...
export const getAllLaborChargeEntriesService = async (status = "ALL", allowedCompanyIds?: number[], fromDate?: string, toDate?: string) => {
  const pool = getPool();
...
export const submitLaborChargeEntriesService = async (sno: number, role = "Administrator") => {
  const pool = getPool();
  if (!pool) throw new Error("Database not connected");
  ...
  try {
    const result = await pool
      ...
      .execute("VPayEntries.SUBMIT_LABOR_CHARGE_ENTRIES");
    ...
    if (/error|not found|already|invalid/i.test(output)) {
      throw new Error(output || "Failed to submit labor charge entries");
    }

    return {
      status: "",
      message: output.includes("Submitted Successfully")
        ? "Labor charge entries submitted successfully"
        : output || "Labor charge entries submitted successfully",
    };
  } catch (error) {
    console.error("SUBMIT_LABOR_CHARGE_ENTRIES SP error:", error);
    throw error;
  }
};
