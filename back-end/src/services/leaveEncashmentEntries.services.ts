...
export const getAllLeaveEncashmentEntriesService = async (status = "ALL", allowedCompanyIds?: number[], fromDate?: string, toDate?: string) => {
  const pool = getPool();
  ...
  try {
    const result = await pool
      ...
      .query(`
        SELECT T.[SNO]
              ,T.[LEAVE_ENCASHMENT_REQUEST_REF_NO]
              ,T.[MONTH_ENTERED]
              ,T.[YEAR_ENTERED]
              ,T.[EMP_ID]
              ,T.[FIRST_NAME]
              ...
              ,T.[CURRENCY_ID]
              ,T.[BALANCE_LEAVE_DAYS]
              ,T.[LEAVE_ENCASHMENT_DAYS]
              ,T.[BASIC_SALARY]
              ,T.[FOT_ALLOWANCE]
              ,T.[ATTENDANCE_ALLOWANCE]
              ,T.[ONE_1YP_ALLOWANCE]
              ,T.[TECHNICAL]
              ,T.[POLYVALENT]
              ,T.[RESPONSIBILITY]
              ...
              ,T.[MISCELLANIES]
              ,T.[NIGHT_ALLOWANCE]
              ,T.[EXTRA1],T.[EXTRA2],T.[EXTRA3],T.[EXTRA4],T.[EXTRA5],T.[EXTRA6]
              ,T.[LEAVE_ENCASHMENT_GROSS_AMOUNT]
              ,T.[PAID_STATUS]
              ,T.[REASON]
              ...
              ,T.[CREATED_DATE]
              ,T.[STATUS_MASTER]
        FROM [VPayEntries].[TBL_LEAVE_ENCASHMENT_ENTRIES] T
        LEFT JOIN [VREQUEST].[TBL_LEAVE_ENCASHMENT_REQUEST] R ON R.LEAVE_ENCASHMENT_REQUEST_REF_NO = T.LEAVE_ENCASHMENT_REQUEST_REF_NO
...
// ─── SUBMIT (VPayEntries.SUBMIT_LEAVE_ENCASHMENT_ENTRIES) ───────────────
export const submitLeaveEncashmentEntriesService = async (refNo: string, role = "Administrator") => {
  const pool = getPool();
  if (!pool) throw new Error("Database not connected");
  ...
  try {
    const result = await pool
      ...
      .execute("VPayEntries.SUBMIT_LEAVE_ENCASHMENT_ENTRIES");
    ...
    if (/error|not found|already|invalid/i.test(output)) {
      throw new Error(output || "Failed to submit leave encashment entries");
    }

    return {
      status: "",
      message: output.includes("Submitted Successfully")
        ? "Leave encashment entries submitted successfully"
        : output || "Leave encashment entries submitted successfully",
    };
  } catch (error) {
    console.error("SUBMIT_LEAVE_ENCASHMENT_ENTRIES SP error:", error);
    throw error;
  }
};
