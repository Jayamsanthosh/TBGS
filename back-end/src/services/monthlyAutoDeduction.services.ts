...
export const submitMonthlyAutoDeductionService = async (mautoRefNo: number, role = "Administrator") => {
  const pool = getPool();
  if (!pool) throw new Error("Database not connected");
  ...
  try {
    const result = await pool
      .request()
      .input("M_AUTO_REF_NO", sql.Int, mautoRefNo)
      .input("Role", sql.VarChar(50), role || "Administrator")
      .execute("VPayEntries.SUBMIT_MONTHLY_AUTO_DEDUCTION");
    ...
    if (/error|not found|already|invalid/i.test(output)) {
      throw new Error(output || "Failed to submit monthly auto deduction");
    }

    return {
      status: "",
      message: output.includes("Submitted Successfully")
        ? "Monthly auto deduction submitted successfully"
        : output || "Monthly auto deduction submitted successfully",
    };
  } catch (error) {
    console.error("SUBMIT_MONTHLY_AUTO_DEDUCTION SP error:", error);
    throw error;
  }
};
