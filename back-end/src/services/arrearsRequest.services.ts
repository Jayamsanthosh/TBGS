...
export interface ArrearsRequestData {
  SNO?: number;
...
const getFieldInputs = (data: ArrearsRequestData) => [
...
export const getAllArrearsRequestsService = async (status = "ALL", allowedCompanyIds?: number[], pendingOnly = false) => {
  const pool = getPool();
  ...
  try {
    ...
    console.error("TBL_ARREARS_REQUEST list query error:", error);
    throw error;
  }
};

export const getArrearsRequestByIdService = async (refNo: string) => {
  const pool = getPool();
  if (!pool) throw new Error("Database not connected");
  ...
  try {
    const result = await pool
      ...
      .execute("VRequest.GET_ARREARS_REQUEST");

    const row = result.recordset[0] || null;
    return row ? normalizeRow(row) : null;
  } catch (error) {
    console.error("GET_ARREARS_REQUEST SP error:", error);
    throw error;
  }
};

export const saveArrearsRequestService = async (data: ArrearsRequestData) => {
  const pool = getPool();
  if (!pool) throw new Error("Database not connected");
  ...
  try {
    ...
    const result = await request.execute("VRequest.SAVE_ARREARS_REQUEST");

    const { status, message, data: savedData } = parseSprocResult(result.recordset?.[0], "Failed to save arrear request");
    ...
    console.error("SAVE_ARREARS_REQUEST SP error:", error);
    throw error;
  }
};

export const updateArrearsRequestService = async (data: ArrearsRequestData) => {
  const pool = getPool();
  if (!pool) throw new Error("Database not connected");
  ...
  try {
    ...
    const result = await request.execute("VRequest.UPDATE_ARREARS_REQUEST");

    const { status, message } = parseSprocResult(result.recordset?.[0], "Failed to update arrear request");

    return { message: message || "Record updated successfully" };
  } catch (error) {
    console.error("UPDATE_ARREARS_REQUEST SP error:", error);
    throw error;
  }
};

export const deleteArrearsRequestService = async (
  refNo: string,
  user = "Admin",
  role = "Admin",
  macAddress = "WEB"
  ...
  try {
    const result = await pool
      ...
      .execute("VRequest.DELETE_ARREARS_REQUEST");

    const { status, message } = parseSprocResult(result.recordset?.[0], "Failed to delete arrear request");

    return { message: message || "Record deleted successfully" };
  } catch (error) {
    console.error("DELETE_ARREARS_REQUEST SP error:", error);
    throw error;
  }
};