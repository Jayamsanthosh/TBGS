...
export const getAllEmployeeDailyShiftDetailsService = async (
  ...
  try {
    ...
    if (empId && String(empId).trim() !== "") {
      where.push("A.[EMP_ID] = @EMP_ID");
    }
    if (status && status !== "ALL") {
      where.push("A.[STATUS_MASTER] = @STATUS");
    }
    where.push("(@FROM_DATE IS NULL OR CAST(A.[CREATED_DATE] AS DATE) >= CAST(@FROM_DATE AS DATE))");
    where.push("(@TO_DATE IS NULL OR CAST(A.[CREATED_DATE] AS DATE) <= CAST(@TO_DATE AS DATE))");

    const query = `${LIST_QUERY}${where.length ? ` WHERE ${where.join(" AND ")}` : ""} ORDER BY A.[SNO] DESC`;

    const req = pool.request();
    if (empId && String(empId).trim() !== "") {
      req.input("EMP_ID", sql.VarChar(50), String(empId));
    }
    if (status && status !== "ALL") {
      req.input("STATUS", sql.VarChar(20), status);
    }
    req.input("FROM_DATE", sql.VarChar(10), fromDate || null);
    req.input("TO_DATE", sql.VarChar(10), toDate || null);
...
export const submitEmployeeDailyShiftDetailsService = async (sno: number, role = "Administrator") => {
  const pool = getPool();
  if (!pool) throw new Error("Database not connected");
  ...
  try {
    const result = await pool
      ...
      .execute("VPayEntries.SUBMIT_EMPLOYEE_DAILY_SHIFT_DETAILS");
    ...
    if (/error|not found|already|invalid/i.test(output)) {
      throw new Error(output || "Failed to submit employee daily shift details");
    }

    return {
      status: "",
      message: output.includes("Submitted Successfully")
        ? "Employee daily shift details submitted successfully"
        : output || "Employee daily shift details submitted successfully",
    };
  } catch (error) {
    console.error("SUBMIT_EMPLOYEE_DAILY_SHIFT_DETAILS SP error:", error);
    throw error;
  }
};
