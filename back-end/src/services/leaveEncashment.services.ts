...
const getFieldInputs = (data: LeaveEncashmentData) => [
...
export const getLeaveEncashmentRequestByIdService = async (refNo: string) => {
  const pool = getPool();
  if (!pool) throw new Error("Database not connected");
  ...
  try {
    const result = await pool
      ...
      .execute("VRequest.GET_LEAVE_ENCASHMENT_REQUEST");

    const row = result.recordset[0] || null;
    return row ? normalizeRow(row) : null;
  } catch (error) {
    console.error("GET_LEAVE_ENCASHMENT_REQUEST SP error:", error);
    throw error;
  }
};

export const showLeaveEncashmentRequestService = async (refNo: string) => {
  const pool = getPool();
  if (!pool) throw new Error("Database not connected");
  ...
  try {
    const result = await pool
      .request()
      .input("LEAVE_ENCASHMENT_REQUEST_REF_NO", sql.VarChar(50), refNo)
      .execute("VRequest.SHOW_LEAVE_ENCASHMENT_REQUEST");

    const row = result.recordset[0] || null;
    return row ? normalizeRow(row) : null;
  } catch (error) {
    console.error("SHOW_LEAVE_ENCASHMENT_REQUEST SP error:", error);
    throw error;
  }
};

const mergeEmployeeAutoFill = (data: LeaveEncashmentData, employee: EmployeePickup): LeaveEncashmentData => {
  return {
    ...data,
    ...
    BALANCE_LEAVE_DAYS:
      numOrNull(data.BALANCE_LEAVE_DAYS) ?? numOrNull(employee.LEAVE),
  } as LeaveEncashmentData;
};

export const saveLeaveEncashmentRequestService = async (data: LeaveEncashmentData) => {
  const pool = getPool();
  if (!pool) throw new Error("Database not connected");
  ...
  try {
    ...
    const result = await request.execute("VRequest.SAVE_LEAVE_ENCASHMENT_REQUEST");

    const { status, message, data: savedData } = parseSprocResult(result.recordset?.[0], "Failed to save leave encashment request");
    ...
    console.error("SAVE_LEAVE_ENCASHMENT_REQUEST SP error:", error);
    throw error;
  }
};

export const updateLeaveEncashmentRequestService = async (data: LeaveEncashmentData) => {
  const pool = getPool();
  if (!pool) throw new Error("Database not connected");

  try {
    const existing = data.LEAVE_ENCASHMENT_REQUEST_REF_NO
      ? await getLeaveEncashmentRequestByIdService(data.LEAVE_ENCASHMENT_REQUEST_REF_NO)
      : null;
    ...
    const result = await request.execute("VRequest.UPDATE_LEAVE_ENCASHMENT_REQUEST");

    const { status, message } = parseSprocResult(result.recordset?.[0], "Failed to update leave encashment request");

    return { message: message || "Record updated successfully" };
  } catch (error) {
    console.error("UPDATE_LEAVE_ENCASHMENT_REQUEST SP error:", error);
    throw error;
  }
};

export const deleteLeaveEncashmentRequestService = async (
  refNo: string,
  user = "Admin",
  role = "Admin",
  macAddress = "WEB"
  ...
  try {
    const result = await pool
      ...
      .execute("VRequest.DELETE_LEAVE_ENCASHMENT_REQUEST");

    const { status, message } = parseSprocResult(result.recordset?.[0], "Failed to delete leave encashment request");

    return { message: message || "Record deleted successfully" };
  } catch (error) {
    console.error("DELETE_LEAVE_ENCASHMENT_REQUEST SP error:", error);
    throw error;
  }
};