...
const pickGunOwnerFields = (row: any) => {
  if (!row) return row;
  let ownerId: any;
  let sourceTypeId: any;

  if (row.GUN_OWNER_BP_ID !== undefined) {
    ownerId = row.GUN_OWNER_BP_ID;
  } else if (Array.isArray(row[""])) {
    ownerId = row[""][0];
  } else {
    const ownerKey = Object.keys(row).find((k) => k.includes("GUN_OWNER_BP_ID"));
    if (ownerKey) ownerId = row[ownerKey];
  }

  if (row.GUN_SOURCE_TYPE_ID !== undefined) {
    sourceTypeId = row.GUN_SOURCE_TYPE_ID;
  } else if (Array.isArray(row[""])) {
    sourceTypeId = row[""][1];
  } else {
    const sourceKey = Object.keys(row).find((k) => k.includes("GUN_SOURCE_TYPE_ID"));
    if (sourceKey) sourceTypeId = row[sourceKey];
  }

  const result: any = { ...row };
  delete result[""];
  for (const k of Object.keys(result)) {
    if (k.includes("isnull(")) delete result[k];
  }
  result.GUN_OWNER_BP_ID = ownerId;
  result.GUN_SOURCE_TYPE_ID = sourceTypeId;
  return result;
};
...
export const getGunMasterByIdService = async (id: number) => {
  ...
  try {
    ...
    return pickGunOwnerFields(result.recordset[0]) || null;
  } catch (error) {
...
export const saveGunMasterService = async (data: GunMasterData) => {
  ...
  try {
    const result = await pool
      ...
      .input("GUN_OWNER_BP_ID", sql.Int, data.GUN_OWNER_BP_ID ?? null)
      .input("GUN_SOURCE_TYPE_ID", sql.Int, data.GUN_SOURCE_TYPE_ID ?? null)
