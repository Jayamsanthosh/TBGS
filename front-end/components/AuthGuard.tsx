...
import { isMenuOpened, ALWAYS_OPEN } from "@/lib/navigationOrigin";
...
export function AuthGuard({ children }: { children: React.ReactNode }) {
  ...
  const isLoading = authLoading || (!isPublic && permLoading);
  ...
  useEffect(() => {
    ...
    if (isAuthenticated && !isPublic && pathname !== "/dashboard" && pathname !== "/" && !allowed) {
      router.replace("/unauthorized");
      return;
    }

    if (isAuthenticated && !isPublic && pathname !== "/dashboard" && pathname !== "/") {
      const isAlwaysOpen = ALWAYS_OPEN.some((p) => (p === "/" ? pathname === "/" : pathname === p || pathname.startsWith(`${p}/`)));
      if (!isAlwaysOpen && !isMenuOpened(pathname)) {
        router.replace("/unauthorized");
      }
    }
  }, [isLoading, isAuthenticated, isPublic, pathname, allowed, router]);
