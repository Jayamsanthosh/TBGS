...
export const fetchEmployeeDailyShiftDetails = createAsyncThunk(
  ...
  async (params: { empId?: string; status?: string; fromDate?: string; toDate?: string }, { rejectWithValue }) => {
    try {
...
export const submitEmployeeDailyShiftDetail = createAsyncThunk(
  "employeeDailyShiftDetails/submit",
  async (sno: string | number, { rejectWithValue, getState }) => {
    try {
      const state: any = getState();
      const authUser = state.auth?.user;
      const Role = authUser?.role || authUser?.ROLE || "Administrator";
      const response = await fetch(`${API_URL}/employee-daily-shift-details/${encodeURIComponent(String(sno))}/submit`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        return rejectWithValue(errorData.message || "Failed to submit daily shift detail");
      }
      return await response.json();
    } catch (error: any) {
      return rejectWithValue(error.message || "Failed to submit daily shift detail");
    }
  }
);
...
const employeeDailyShiftDetailsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(submitEmployeeDailyShiftDetail.pending, (state) => { state.error = null; })
      .addCase(submitEmployeeDailyShiftDetail.rejected, (state, action) => {
        state.error = (action.payload as string) || action.error.message || "Failed to submit daily shift detail";
      })
