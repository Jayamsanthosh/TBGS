...
export const fetchAttendanceDetails = createAsyncThunk(
  ...
  async (args: { status?: string; fromDate?: string; toDate?: string } = {}, { rejectWithValue }) => {
    try {
      ...
      const response = await fetch(`${API_URL}/attendance-details?${query.toString()}`);
      if (!response.ok) {
...
export const submitAttendanceDetail = createAsyncThunk(
  "attendanceDetails/submitAttendanceDetail",
  async (item: AttendanceDetailsGridData, { rejectWithValue }) => {
    try {
      const id = Number(item.id ?? item.SNO);
      const response = await fetch(`${API_URL}/attendance-details/${id}/submit`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(item),
      });
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        return rejectWithValue(errorData.message || "Failed to submit attendance detail");
      }
      return await response.json();
    } catch (error: any) {
      return rejectWithValue(error.message || "Failed to submit attendance detail");
    }
  }
);
...
const attendanceDetailsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(submitAttendanceDetail.pending, (state) => { state.error = null; })
      .addCase(submitAttendanceDetail.rejected, (state, action) => {
        state.error = (action.payload as string) || action.error.message || "Failed to submit attendance detail";
      });
  },
});
