...
export const fetchDeductionEntries = createAsyncThunk(
  ...
  async (args: { status?: string; fromDate?: string; toDate?: string } = {}, { rejectWithValue }) => {
    try {
      ...
      const response = await fetch(`${API_URL}/deduction-entries?${query.toString()}`);
      if (!response.ok) {
...
export const submitDeductionEntries = createAsyncThunk(
  "deductionEntries/submitDeductionEntries",
  async (item: DeductionEntriesGridData, { rejectWithValue, getState }) => {
    try {
      const refNo = item.DED_REF_ID;
      const state: any = getState();
      const authUser = state.auth?.user;
      const Role = authUser?.role || authUser?.ROLE || "Administrator";
      const response = await fetch(`${API_URL}/deduction-entries/${encodeURIComponent(String(refNo))}/submit`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        return rejectWithValue(errorData.message || "Failed to submit deduction entries");
      }
      return await response.json();
    } catch (error: any) {
      return rejectWithValue(error.message || "Failed to submit deduction entries");
    }
  }
);
...
const deductionEntriesSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(submitDeductionEntries.pending, (state) => {
        state.error = null;
      })
      .addCase(submitDeductionEntries.rejected, (state, action) => {
        state.error = (action.payload as string) || action.error.message || "Failed to submit deduction entries";
      })
