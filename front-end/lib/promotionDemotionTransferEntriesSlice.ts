  ...
  async (args: { status?: string; fromDate?: string; toDate?: string } = {}, { rejectWithValue }) => {
    try {
      ...
      const response = await fetch(
        `${API_URL}/promotion-demotion-transfer-entries?${query.toString()}`
      );
      if (!response.ok) {
...
export const submitPromotionDemotionTransferEntry = createAsyncThunk(
  "promotionDemotionTransferEntries/submitPromotionDemotionTransferEntry",
  async (item: PromotionDemotionTransferEntriesGridData, { rejectWithValue, getState }) => {
    try {
      const refNo = item.TRANSFER_REQUEST_REF_NO;
      const state: any = getState();
      const authUser = state.auth?.user;
      const Role = authUser?.role || authUser?.ROLE || "Administrator";
      const response = await fetch(`${API_URL}/promotion-demotion-transfer-entries/${encodeURIComponent(String(refNo))}/submit`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        return rejectWithValue(errorData.message || "Failed to submit promotion / demotion / transfer entries");
      }
      return await response.json();
    } catch (error: any) {
      return rejectWithValue(error.message || "Failed to submit promotion / demotion / transfer entries");
    }
  }
);
...
const promotionDemotionTransferEntriesSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(submitPromotionDemotionTransferEntry.pending, (state) => { state.error = null; })
      .addCase(submitPromotionDemotionTransferEntry.rejected, (state, action) => {
        state.error = (action.payload as string) || action.error.message || "Failed to submit promotion / demotion / transfer entries";
      })
