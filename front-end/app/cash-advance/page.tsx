...
export default function CashAdvancePage() {
  ...
  const fields: MasterField[] = [
    { key: "MONTH_ENTERED", label: "Month", type: "select", options: MONTHS, disabled: true, placeholder: "Select month" },
    { key: "YEAR_ENTERED", label: "Year", type: "select", options: YEARS, disabled: true, placeholder: "Select year" },
    {
      key: "EMP_ID",
  ...
  const handleFieldChange = useCallback((key: string, value: any, setForm: any, form: Record<string, any>): boolean => {
    if (key === "CASH_ADV_REQUEST_REF_NO") {
      ...
      if (req) {
        ...
        setForm({ ...form, CASH_ADV_REQUEST_REF_NO: value });
      }
      return true;
    }
  ...
  const storeOverrides = useMemo(() => ({
    ...
    add: async (item: CashAdvanceGridData) => {
      const res = await dispatch(addCashAdvance(cleanPayload(item))).unwrap();
      dispatch(fetchCashAdvances(currentStatus));
      return res;
    },
  ...
  return (
    <MasterCrudPage
      title="Cash Advance"
      description="Manage cash advance entries"
      idPrefix="CA"
      domain="cash-advance"
      fields={fields}
      columns={columns}
