...
import { fetchParticipantTypes, addParticipantType, updateParticipantType, deleteParticipantType, clearParticipantTypeError, type ParticipantTypeGridData } from "@/lib/participantTypeMasterSlice";
...
export default function ParticipantTypeMasterPage() {
  const dispatch = useAppDispatch();
  const { participantTypes, loading, error } = useAppSelector((s) => s.participantType);
  ...
  const filtered = useMemo(() => {
    if (!Array.isArray(participantTypes)) return [];
    return participantTypes.filter((d: any) => {
      const searchable = [d.PARTICIPANT_TYPE_NAME, d.PARTICIPANT_TYPE_DESCRIPTION, d.REMARKS, d.STATUS_MASTER].join(" ").toLowerCase();
      ...
      return true;
    }).sort((a: any, b: any) => Number(b.PARTICIPANT_TYPE_ID) - Number(a.PARTICIPANT_TYPE_ID));
  }, [participantTypes, search, statusFilter]);
  ...
  const openEdit = (item: any) => {
    setEditing(item);
    setForm({
      PARTICIPANT_TYPE_ID: item.PARTICIPANT_TYPE_ID || 0,
      PARTICIPANT_TYPE_NAME: item.PARTICIPANT_TYPE_NAME || "",
      PARTICIPANT_TYPE_DESCRIPTION: item.PARTICIPANT_TYPE_DESCRIPTION || "",
  ...
  const handleSave = async () => {
    if (!form.PARTICIPANT_TYPE_NAME?.trim()) {
      toast({ title: "Participant type name is required", variant: "destructive", duration: DEFAULT_TOAST_DURATION });
      return;
    }
    if (form.PARTICIPANT_TYPE_NAME.trim().length > 50) {
      toast({ title: "Participant type name must be 50 characters or less", variant: "destructive", duration: DEFAULT_TOAST_DURATION });
      return;
    }
    if (form.PARTICIPANT_TYPE_DESCRIPTION && form.PARTICIPANT_TYPE_DESCRIPTION.trim().length > 50) {
    ...
    if (!editing && normalizeStatus(form.STATUS_MASTER) === "INACTIVE") {
      toast({ title: "New participant type cannot be set to Inactive", variant: "destructive", duration: DEFAULT_TOAST_DURATION });
      return;
    }

    try {
      const payload: Record<string, any> = {
        PARTICIPANT_TYPE_NAME: form.PARTICIPANT_TYPE_NAME.trim(),
        PARTICIPANT_TYPE_DESCRIPTION: form.PARTICIPANT_TYPE_DESCRIPTION?.trim() || null,
      ...
      if (editing) {
        payload.PARTICIPANT_TYPE_ID = Number(editing.id);
        const res = await dispatch(updateParticipantType(payload as ParticipantTypeGridData)).unwrap();
        toast({ title: res?.message ?? "Participant type updated successfully", duration: DEFAULT_TOAST_DURATION });
      } else {
        const res = await dispatch(addParticipantType(payload as ParticipantTypeGridData)).unwrap();
        toast({ title: res?.message ?? "Participant type created successfully", duration: DEFAULT_TOAST_DURATION });
      }
      setDialogOpen(false);
      dispatch(fetchParticipantTypes(statusFilter));
    } catch (e: any) {
      toast({ title: typeof e === "string" ? e : (e?.message || "Error saving participant type"), variant: "destructive", duration: DEFAULT_TOAST_DURATION });
    }
  };
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      const res = await dispatch(deleteParticipantType(deleteId)).unwrap();
      setDeleteId(null);
      toast({ title: res?.message ?? "Participant type deleted successfully", duration: DEFAULT_TOAST_DURATION });
      dispatch(fetchParticipantTypes(statusFilter));
    } catch (e: any) {
      toast({ title: typeof e === "string" ? e : (e?.message || "Error deleting participant type"), variant: "destructive", duration: DEFAULT_TOAST_DURATION });
    }
  };
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Participant Type Master</h1>
          <p className="text-sm text-muted-foreground">Manage participant type master data</p>
        </div>
        <Button onClick={openAdd} className="bg-primary text-primary-foreground hover:bg-primary/90">
          <Plus className="w-4 h-4 mr-2" /> Add Participant Type
        </Button>
      </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
          <div className="flex items-center gap-2 w-full sm:max-w-md">
            <div className="relative flex-1">
              ...
              <Input placeholder="Search participant types..." value={search} onChange={(e) => { setSearch(e.target.value); setCurrentPage(1); }} className="pl-9 h-9 text-xs" />
            </div>
              ...
              <tbody>
                {paginated.map((item: any, idx: number) => (
                  <tr key={item.id || idx} className="border-b hover:bg-muted/30 transition-colors">
                    ...
                    <td className="p-3 font-medium">{item.PARTICIPANT_TYPE_ID}</td>
                    <td className="p-3">{item.PARTICIPANT_TYPE_NAME}</td>
                    <td className="p-3">{item.PARTICIPANT_TYPE_DESCRIPTION || "-"}</td>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editing ? "Edit Participant Type" : "Add Participant Type"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-6">
            <div className="grid grid-cols-2 gap-4">
              {renderField("PARTICIPANT_TYPE_NAME", "Participant Type Name", "text", undefined, true, "e.g., Hunter", 50)}
              {renderField("PARTICIPANT_TYPE_DESCRIPTION", "Description", "text", undefined, false, "e.g., Person who hunts", 50)}
      ...
      <AlertDialog open={!!deleteId} onOpenChange={(open) => !open && setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>This will permanently delete this participant type.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
