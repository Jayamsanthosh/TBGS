...
import { fetchExpensePayerTypes, addExpensePayerType, updateExpensePayerType, deleteExpensePayerType, clearExpensePayerTypeError, type ExpensePayerTypeGridData } from "@/lib/expensePayerTypeMasterSlice";
...
export default function ExpensePayerTypeMasterPage() {
  const dispatch = useAppDispatch();
  const { expensePayerTypes, loading, error } = useAppSelector((s) => s.expensePayerType);
  ...
  const filtered = useMemo(() => {
    if (!Array.isArray(expensePayerTypes)) return [];
    return expensePayerTypes.filter((d: any) => {
      const searchable = [d.PAID_BY_NAME, d.PAID_BY_DESCRIPTION, d.REMARKS, d.STATUS_MASTER].join(" ").toLowerCase();
      ...
      return true;
    }).sort((a: any, b: any) => Number(b.PAID_BY_ID) - Number(a.PAID_BY_ID));
  }, [expensePayerTypes, search, statusFilter]);
  ...
  const openEdit = (item: any) => {
    setEditing(item);
    setForm({
      PAID_BY_ID: item.PAID_BY_ID || 0,
      PAID_BY_NAME: item.PAID_BY_NAME || "",
      PAID_BY_DESCRIPTION: item.PAID_BY_DESCRIPTION || "",
  ...
  const handleSave = async () => {
    if (!form.PAID_BY_NAME?.trim()) {
      toast({ title: "Paid By Name is required", variant: "destructive", duration: DEFAULT_TOAST_DURATION });
      return;
    }
    if (form.PAID_BY_NAME.trim().length > 50) {
      toast({ title: "Paid By Name must be 50 characters or less", variant: "destructive", duration: DEFAULT_TOAST_DURATION });
      return;
    }
    if (form.PAID_BY_DESCRIPTION && form.PAID_BY_DESCRIPTION.trim().length > 50) {
    ...
    if (!editing && normalizeStatus(form.STATUS_MASTER) === "INACTIVE") {
      toast({ title: "New expense payer type cannot be set to Inactive", variant: "destructive", duration: DEFAULT_TOAST_DURATION });
      return;
    }

    try {
      const payload: Record<string, any> = {
        PAID_BY_NAME: form.PAID_BY_NAME.trim(),
        PAID_BY_DESCRIPTION: form.PAID_BY_DESCRIPTION?.trim() || null,
      ...
      if (editing) {
        payload.PAID_BY_ID = Number(editing.id);
        const res = await dispatch(updateExpensePayerType(payload as ExpensePayerTypeGridData)).unwrap();
        toast({ title: res?.message ?? "Expense payer type updated successfully", duration: DEFAULT_TOAST_DURATION });
      } else {
        const res = await dispatch(addExpensePayerType(payload as ExpensePayerTypeGridData)).unwrap();
        toast({ title: res?.message ?? "Expense payer type created successfully", duration: DEFAULT_TOAST_DURATION });
      }
      setDialogOpen(false);
      dispatch(fetchExpensePayerTypes(statusFilter));
    } catch (e: any) {
      toast({ title: typeof e === "string" ? e : (e?.message || "Error saving expense payer type"), variant: "destructive", duration: DEFAULT_TOAST_DURATION });
    }
  };
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      const res = await dispatch(deleteExpensePayerType(deleteId)).unwrap();
      setDeleteId(null);
      toast({ title: res?.message ?? "Expense payer type deleted successfully", duration: DEFAULT_TOAST_DURATION });
      dispatch(fetchExpensePayerTypes(statusFilter));
    } catch (e: any) {
      toast({ title: typeof e === "string" ? e : (e?.message || "Error deleting expense payer type"), variant: "destructive", duration: DEFAULT_TOAST_DURATION });
    }
  };
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Expense Payer Type Master</h1>
          <p className="text-sm text-muted-foreground">Manage expense payer type master data</p>
        </div>
        <Button onClick={openAdd} className="bg-primary text-primary-foreground hover:bg-primary/90">
          <Plus className="w-4 h-4 mr-2" /> Add Expense Payer Type
        </Button>
      </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
          <div className="flex items-center gap-2 w-full sm:max-w-md">
            <div className="relative flex-1">
              ...
              <Input placeholder="Search expense payer types..." value={search} onChange={(e) => { setSearch(e.target.value); setCurrentPage(1); }} className="pl-9 h-9 text-xs" />
            </div>
              ...
              <tbody>
                {paginated.map((item: any, idx: number) => (
                  <tr key={item.id || idx} className="border-b hover:bg-muted/30 transition-colors">
                    ...
                    <td className="p-3 font-medium">{item.PAID_BY_ID}</td>
                    <td className="p-3">{item.PAID_BY_NAME}</td>
                    <td className="p-3">{item.PAID_BY_DESCRIPTION || "-"}</td>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editing ? "Edit Expense Payer Type" : "Add Expense Payer Type"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-6">
            <div className="grid grid-cols-2 gap-4">
              {renderField("PAID_BY_NAME", "Paid By Name", "text", undefined, true, "e.g., Customer", 50)}
              {renderField("PAID_BY_DESCRIPTION", "Description", "text", undefined, false, "e.g., Paid by customer", 50)}
      ...
      <AlertDialog open={!!deleteId} onOpenChange={(open) => !open && setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>This will permanently delete this expense payer type.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
