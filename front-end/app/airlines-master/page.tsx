...
import { fetchAirlines, addAirlines, updateAirlines, deleteAirlines, clearAirlinesError, type AirlinesGridData } from "@/lib/airlinesMasterSlice";
...
export default function AirlinesMasterPage() {
  ...
  const { airlines, loading, error } = useAppSelector((s) => s.airlines);
  ...
  const filtered = useMemo(() => {
    ...
    return airlines.filter((d: any) => {
      const searchable = [d.AIRLINE_NAME, d.REMARKS, d.STATUS_MASTER].join(" ").toLowerCase();
      ...
      return true;
    }).sort((a: any, b: any) => Number(b.AIRLINE_ID) - Number(a.AIRLINE_ID));
  ...
  const handleSave = async () => {
    if (!form.AIRLINE_NAME?.trim()) {
      toast({ title: "Airline name is required", variant: "destructive", duration: DEFAULT_TOAST_DURATION });
      return;
    }
    if (form.AIRLINE_NAME.trim().length > 10) {
      toast({ title: "Airline name must be 10 characters or less", variant: "destructive", duration: DEFAULT_TOAST_DURATION });
      return;
    }
    if (!editing && normalizeStatus(form.STATUS_MASTER) === "INACTIVE") {
      toast({ title: "New airline cannot be set to Inactive", variant: "destructive", duration: DEFAULT_TOAST_DURATION });
      return;
    }
    ...
    try {
      ...
      if (editing) {
        payload.AIRLINE_ID = Number(editing.id);
        const res = await dispatch(updateAirlines(payload as AirlinesGridData)).unwrap();
        toast({ title: res?.message ?? "Airline updated successfully", duration: DEFAULT_TOAST_DURATION });
      } else {
        const res = await dispatch(addAirlines(payload as AirlinesGridData)).unwrap();
        toast({ title: res?.message ?? "Airline created successfully", duration: DEFAULT_TOAST_DURATION });
      }
      setDialogOpen(false);
      dispatch(fetchAirlines(statusFilter));
    } catch (e: any) {
      toast({ title: typeof e === "string" ? e : (e?.message || "Error saving airline"), variant: "destructive", duration: DEFAULT_TOAST_DURATION });
    }
  };
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      const res = await dispatch(deleteAirlines(deleteId)).unwrap();
      setDeleteId(null);
      toast({ title: res?.message ?? "Airline deleted successfully", duration: DEFAULT_TOAST_DURATION });
      dispatch(fetchAirlines(statusFilter));
    } catch (e: any) {
      toast({ title: typeof e === "string" ? e : (e?.message || "Error deleting airline"), variant: "destructive", duration: DEFAULT_TOAST_DURATION });
    }
  };
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Airlines Master</h1>
          <p className="text-sm text-muted-foreground">Manage airlines master data</p>
        </div>
        <Button onClick={openAdd} className="bg-primary text-primary-foreground hover:bg-primary/90">
          <Plus className="w-4 h-4 mr-2" /> Add Airline
        </Button>
      </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
          <div className="flex items-center gap-2 w-full sm:max-w-md">
            <div className="relative flex-1">
              ...
              <Input placeholder="Search airlines..." value={search} onChange={(e) => { setSearch(e.target.value); setCurrentPage(1); }} className="pl-9 h-9 text-xs" />
            </div>
              ...
              <tbody>
                {paginated.map((item: any, idx: number) => (
                  <tr key={item.id || idx} className="border-b hover:bg-muted/30 transition-colors">
                    ...
                    <td className="p-3 font-medium">{item.AIRLINE_ID}</td>
                    <td className="p-3">{item.AIRLINE_NAME}</td>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editing ? "Edit Airline" : "Add Airline"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-6">
            <div className="grid grid-cols-2 gap-4">
              {renderField("AIRLINE_NAME", "Airline Name", "text", undefined, true, "e.g., Indigo", 10)}
      ...
      <AlertDialog open={!!deleteId} onOpenChange={(open) => !open && setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>This will permanently delete this airline.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
