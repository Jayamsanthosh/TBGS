...
export default function OvertimeEntriesPage() {
  ...
  const handleSubmit = useCallback(async (item: any) => {
    ...
    try {
      const res = await dispatch(submitOvertimeEntries({ ...item, OT_REQUEST_REF_NO: ref } as OvertimeEntriesGridData)).unwrap();
      toast({ title: res?.message ?? "Overtime entry submitted successfully!" });
      dispatch(fetchOvertimeEntries({ status: currentStatus, fromDate, toDate }));
    } catch (e: any) {
      const msg = typeof e === "string" ? e : (e?.message || "Failed to submit overtime entry");
  ...
  return (
    <div className="space-y-4">
      ...
      description="Manage overtime responses"
      idPrefix="OT"
