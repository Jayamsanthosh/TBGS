...
import {
  ...
  submitEmployeeDailyShiftDetail,
...
// Top inputs
  ...
  const { data: employees } = useApiQuery("edsd-employees", async () => {
    const res = await fetch(`${API_URL}/employee-database?status=AC`);
    return (await res.json()).data || [];
  });
  const { data: allEmployees } = useApiQuery("edsd-all-employees", async () => {
    const res = await fetch(`${API_URL}/employee-database?status=ALL`);
    return (await res.json()).data || [];
  });

  // Check whether the selected employee is stacked/duplicated against another employee record
  const duplicateWarnings = useMemo(() => {
    if (!fullEmp || !allEmployees || allEmployees.length === 0) return [];
    const selId = String(fullEmp.EMP_ID ?? "").trim();
    const selName = [fullEmp.FIRST_NAME, fullEmp.MIDDLE_NAME, fullEmp.LAST_NAME].filter(Boolean).join(" ").toLowerCase();
    const others = allEmployees.filter((e: any) => String(e.SNO ?? "") !== String(fullEmp.SNO ?? ""));
    const warnings: string[] = [];

    const sameId = others.filter((e: any) => String(e.EMP_ID ?? "") === selId && String(e.EMP_ID ?? "") !== "");
    sameId.forEach((e: any) => {
      const name = [e.FIRST_NAME, e.MIDDLE_NAME, e.LAST_NAME].filter(Boolean).join(" ");
      warnings.push(`Employee ID ${selId} is also stacked with Employee ${e.SNO} (${name || "Name not available"}) - emp # ${e.EMP_ID}.`);
    });

    if (selName) {
      const sameName = others.filter((e: any) => {
        if (String(e.EMP_ID ?? "") === selId && String(e.EMP_ID ?? "") !== "") return false;
        const nm = [e.FIRST_NAME, e.MIDDLE_NAME, e.LAST_NAME].filter(Boolean).join(" ").toLowerCase();
        return nm === selName && nm !== "";
      });
      sameName.forEach((e: any) => {
        warnings.push(`Name "${selName}" is also present under another employee number (EMP ID ${e.EMP_ID}, SNO ${e.SNO}).`);
      });
    }

    return [...new Set(warnings)];
  }, [fullEmp, allEmployees]);
  ...
  useEffect(() => {
    const fetchEmp = async () => {
      ...
      try {
        ...
        const term = String(empIdInput).trim();
        const empGrid = list.find((e: any) => String(e.EMP_ID) === term)
          || list.find((e: any) => String(e.EMP_ID).startsWith(term))
          || list.find((e: any) => {
              const nm = [e.FIRST_NAME, e.MIDDLE_NAME, e.LAST_NAME].filter(Boolean).join(" ").toLowerCase();
              return nm.includes(term.toLowerCase());
            });
  ...
  const isLockedRecord = (rec: any): boolean => {
    if (!rec) return false;
    const sv = String(rec.STATUS_MASTER || "").toUpperCase();
    return sv === "CL" || sv === "CA";
  };

  const handleSelectAll = (checked: boolean) => {
    if (checked) {
      const selectable = daysInMonth.filter((d) => {
        const rec = items.find((it: any) => it.SHIFT_DATE && it.SHIFT_DATE.startsWith(d.isoDate));
        return !isLockedRecord(rec);
      });
      setSelectedDates(new Set(selectable.map(d => d.dayNumber)));
  ...
  const handleSubmitTransaction = async () => {
    ...
    const pending = items.filter((it: any) => {
      const sameMonth = String(it.SHIFT_MONTH).toUpperCase() === String(month).toUpperCase();
      const sameYear = String(it.SHIFT_YEAR) === String(year);
      const st = String(it.STATUS_MASTER || "").toUpperCase();
      return sameMonth && sameYear && st !== "CL" && st !== "CA";
    });

    if (pending.length === 0) {
      toast({ title: "No pending shift details to submit for this month", variant: "destructive", duration: DEFAULT_TOAST_DURATION });
      return;
    }
    ...
    try {
      ...
      for (const it of pending) {
        if (!it.SNO) continue;
        const res = await dispatch(submitEmployeeDailyShiftDetail(Number(it.SNO))).unwrap();
        lastMsg = res?.message || lastMsg;
        submitted++;
      }
      if (submitted === 0) {
        toast({ title: "No shift details could be submitted", variant: "destructive", duration: DEFAULT_TOAST_DURATION });
        return;
      }
      toast({ title: `${submitted} shift detail(s) submitted successfully`, duration: DEFAULT_TOAST_DURATION });
      setSelectedDates(new Set());
      setRowShiftId({});
      dispatch(fetchEmployeeDailyShiftDetails({ empId: String(fullEmp.EMP_ID) }));
    } catch (e: any) {
      const msg = typeof e === "string" ? e : (e?.message || "Failed to submit shift details");
      ...
      <div className="border border-border shadow-sm rounded-md bg-background min-w-[900px]">
        ...
        <div className="grid grid-cols-5 gap-0 border-b">
          <div className="p-2 border-r flex flex-col gap-1 bg-muted/20">
            <Label className="text-xs text-muted-foreground font-semibold">Employee (Name / ID)</Label>
            <EmployeeCombobox
              value={empIdInput}
              options={employees || []}
              onChange={(v) => setEmpIdInput(v)}
            />
          </div>
          <div className="p-2 border-r flex flex-col gap-1 bg-muted/20">
        ...
        {/* Duplicate / Stacked Employee Warning */}
        {duplicateWarnings.length > 0 && (
          <div className="border-b px-3 py-2">
            <Alert variant="destructive" className="py-2">
              <AlertTriangle className="h-4 w-4" />
              <AlertTitle className="text-sm">Duplicate / Stacked Employee Detected</AlertTitle>
              <AlertDescription>
        ...
        {showTable && (
            ...
            <table className="text-xs text-left border-collapse w-full">
              <thead>
                <tr className="bg-[#1a5f4d] text-white">
                  ...
                  <th className="p-2 border-r border-white/20 font-semibold min-w-32">SHIFT</th>
                </tr>
              </thead>
                  ...
                  <tr key={row.dateStr} className={`border-b border-border ${idx % 2 === 0 ? "bg-background" : "bg-muted/30"}`}>
                    <td className="p-2 border-r border-border text-center">
                      <Checkbox 
                        ...
                        disabled={isLockedRecord(row.existingRecord)}
          ...
          <div className="mt-4 flex gap-2">
            ...
            <Button className="bg-[#1a5f4d] hover:bg-[#1a5f4d]/90 text-white rounded-sm font-semibold h-8" onClick={handleSubmitTransaction}>
              Submit
            </Button>
          </div>
        </div>
