...
export default function EmployeeBenefitMasterPage() {
  ...
  const addDtl = () => {
    setDtls((prev) => [...prev, emptyDtl(form.EMP_ID)]);
  };
  ...
  const handleEmpSelect = (empId: string) => {
    ...
    setForm((prev) => ({ ...prev, EMP_ID: empId, FIRST_NAME: "", MIDDLE_NAME: "", LAST_NAME: "", COMPANY_ID: "", DEPARTMENT_ID: "", DESIGNATION_ID: "", DEPARTMENT_GROUP_ID: "", DESIGNATION_GROUP_ID: "", CAMP_ID: "", STORE_ID: "", EMPLOYMENT_TYPE_ID: "", CURRENCY_ID: "" }));
      ...
      fetch(`${API_URL}/employee-database/${empGrid.SNO}`)
        ...
        .then(json => {
          if (json.success && json.data) {
            ...
            setDtls((prev) => prev.length === 0 ? [emptyDtl(empId)] : prev.map((r: any) => ({ ...r, EMP_ID: Number(empId) })));
          }
        });
    } else {
  ...
  const openEdit = async (item: any) => {
    ...
    try {
      ...
      setDtls(rows.length ? rows : [emptyDtl(hdr.EMP_ID)]);
      setDeletedIds([]);
    } catch (e: any) {
                  ...
                  <div key={row.key} className="border rounded-lg p-3 space-y-3">
                    <div className="flex flex-wrap gap-3 items-start">
                      ...
                      <div className="flex flex-col gap-1 w-28">
                        <Label className="text-[11px] font-medium text-muted-foreground">Gross Amount</Label>
                        <Input type="number" min="0" step="any" value={row.GROSS_AMOUNT} onChange={(e) => updateDtl(row.key, "GROSS_AMOUNT", clampNonNegative(e.target.value))} className="h-8 text-xs" />
                      </div>
                      <div className="flex flex-col gap-1 w-28">
                        <Label className="text-[11px] font-medium text-muted-foreground">Paid Status</Label>
                    ...
                    <div className="flex gap-3 items-center">
                      ...
                      <button type="button" onClick={() => removeDtl(row.key)} className="p-1.5 rounded hover:bg-destructive/10 transition-colors self-end">
