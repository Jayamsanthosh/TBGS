...
import { fetchBulletTypes, addBulletType, updateBulletType, deleteBulletType, clearBulletTypeError } from "@/lib/bulletTypeMasterSlice";
import type { BulletTypeGridData } from "@/lib/bulletTypeMasterSlice";
...
export default function BulletTypeMasterPage() {
  const dispatch = useAppDispatch();
  const { records, loading, error } = useAppSelector((s) => s.bulletType);
  ...
  const filtered = useMemo(() => {
    ...
    return records.filter((d: any) => {
      const searchable = [d.BULLET_TYPE_NAME, d.DESCRIPTION, d.REMARKS].join(" ").toLowerCase();
      return searchable.includes(search.toLowerCase());
    }).sort((a: any, b: any) => (b.BULLET_TYPE_ID ?? b.id ?? 0) - (a.BULLET_TYPE_ID ?? a.id ?? 0));
  }, [records, search]);
  ...
  const openEdit = (item: any) => {
    setEditing(item);
    setForm({
      BULLET_TYPE_NAME: item.BULLET_TYPE_NAME || "",
  ...
  const handleSave = async () => {
    if (!form.BULLET_TYPE_NAME?.trim()) {
      toast({ title: "Bullet type name is required", variant: "destructive", duration: DEFAULT_TOAST_DURATION });
      return;
    }
    if (form.BULLET_TYPE_NAME.trim().length > 100) {
      toast({ title: "Bullet type name must be 100 characters or less", variant: "destructive", duration: DEFAULT_TOAST_DURATION });
      return;
    }

    try {
      const payload: Record<string, any> = {
        BULLET_TYPE_NAME: form.BULLET_TYPE_NAME.trim(),
      ...
      if (editing) {
        payload.BULLET_TYPE_ID = Number(editing.id);
        const res = await dispatch(updateBulletType(payload as BulletTypeGridData)).unwrap();
        toast({ title: res?.message ?? "Bullet type updated successfully", duration: DEFAULT_TOAST_DURATION });
      } else {
        const res = await dispatch(addBulletType(payload as BulletTypeGridData)).unwrap();
        toast({ title: res?.message ?? "Bullet type created successfully", duration: DEFAULT_TOAST_DURATION });
      }
      setDialogOpen(false);
      dispatch(fetchBulletTypes(statusFilter));
    } catch (e: any) {
      toast({ title: typeof e === "string" ? e : (e?.message || "Error saving bullet type"), variant: "destructive", duration: DEFAULT_TOAST_DURATION });
    }
  };
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      const res = await dispatch(deleteBulletType(deleteId)).unwrap();
      setDeleteId(null);
      toast({ title: res?.message ?? "Bullet type deleted successfully", duration: DEFAULT_TOAST_DURATION });
      dispatch(fetchBulletTypes(statusFilter));
    } catch (e: any) {
      toast({ title: typeof e === "string" ? e : (e?.message || "Error deleting bullet type"), variant: "destructive", duration: DEFAULT_TOAST_DURATION });
    }
  };
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Bullet Type Master</h1>
          <p className="text-sm text-muted-foreground">Manage bullet type master data</p>
        </div>
        <Button onClick={openAdd} className="bg-primary text-primary-foreground hover:bg-primary/90">
          <Plus className="w-4 h-4 mr-2" /> Add Bullet Type
        </Button>
      </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
          <div className="flex items-center gap-2 w-full sm:max-w-md">
            <div className="relative flex-1">
              ...
              <Input placeholder="Search bullet types..." value={search} onChange={(e) => { setSearch(e.target.value); setCurrentPage(1); }} className="pl-9 h-9 text-xs" />
            </div>
              ...
              <tbody>
                {paginated.map((item: any, idx: number) => (
                  <tr key={item.id || idx} className="border-b hover:bg-muted/30 transition-colors">
                    ...
                    <td className="p-3 font-medium">{item.BULLET_TYPE_ID}</td>
                    <td className="p-3">{item.BULLET_TYPE_NAME}</td>
                    <td className="p-3">{item.DESCRIPTION || "-"}</td>
                    <td className="p-3">{item.REMARKS || "-"}</td>
                    <td className="p-3">
                      <Badge variant="outline" className={`${statusBadgeClass(item.STATUS_MASTER)} px-2 py-0.5 text-[10px] uppercase font-bold`}>
                        {formatStatus(item.STATUS_MASTER)}
                      </Badge>
                    </td>
                  </tr>
                ))}
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editing ? "Edit Bullet Type" : "Add Bullet Type"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-6">
            <div className="grid grid-cols-2 gap-4">
              {renderField("BULLET_TYPE_NAME", "Bullet Type Name", "text", undefined, true, "e.g., Full Metal Jacket", 100)}
              {renderField("STATUS_MASTER", "Status", "select", [{ value: "AC", label: "Active" }, { value: "IA", label: "Inactive" }], false)}
            </div>
      ...
      <AlertDialog open={!!deleteId} onOpenChange={(open) => !open && setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>This will permanently delete this bullet type.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
