...
import {
  ...
  updateStatus,
  deleteStatus,
  clearStatusMasterError,
  StatusMasterGridData,
} from "@/lib/statusMasterSlice";
...
export default function StatusMasterPage() {
  ...
  const { items, total, loading, error } = useAppSelector((s) => s.statusMaster);
  ...
  const [editing, setEditing] = useState<StatusMasterGridData | null>(null);
  ...
  const uniqueCategories = useMemo(() => {
    const set = new Set<string>();
    data.forEach((d: any) => {
      const c = d.statusCategory || d.STATUS_CATEGORY;
  ...
  const filtered = useMemo(() => {
    return data.filter((d: any) => {
      ...
      const category = d.statusCategory || d.STATUS_CATEGORY;
      const matchesCategory = categoryFilter === "ALL" || String(category || "") === categoryFilter;
      if (!matchesCategory) return false;
      const searchable = [d.statusCode, d.STATUS_CODE, d.statusName, d.STATUS_NAME, d.statusCategory, d.STATUS_CATEGORY, d.description, d.DESCRIPTION, d.remarks, d.REMARKS]
        .join(" ")
        .toLowerCase();
      ...
      return Number(b.id || b.statusId) - Number(a.id || a.statusId);
    });
  ...
  useEffect(() => {
    if (error) {
      ...
      dispatch(clearStatusMasterError());
    }
  ...
  const openEdit = async (item: any) => {
    ...
    const detail = item && item.id != null
      ? await dispatch(fetchStatusById(Number(item.id))).unwrap().catch(() => null)
      : null;
    const src: any = detail ? detail : item;
    setForm({
      STATUS_CODE: pickVal(src, ["statusCode", "STATUS_CODE"]),
      STATUS_NAME: pickVal(src, ["statusName", "STATUS_NAME"]),
      STATUS_CATEGORY: pickVal(src, ["statusCategory", "STATUS_CATEGORY"]),
  ...
  const handleSave = async () => {
    if (!form.STATUS_CODE?.trim()) {
      toast({ title: "Status Code is required", variant: "destructive", duration: DEFAULT_TOAST_DURATION });
      return;
    }
    if (!form.STATUS_NAME?.trim()) {
      toast({ title: "Status Name is required", variant: "destructive", duration: DEFAULT_TOAST_DURATION });
      return;
    }

    if (!editing && normalizeStatus(form.STATUS_MASTER) === "INACTIVE") {
      toast({ title: "New status cannot be set to Inactive", variant: "destructive", duration: DEFAULT_TOAST_DURATION });
      return;
    }
    ...
    try {
      ...
      const payload: Record<string, any> = {
        ...
        STATUS_CATEGORY: form.STATUS_CATEGORY?.trim() || null,
      ...
      if (editing) {
        ...
        payload.STATUS_ID = Number(editing.id) || editing.statusId;
        const res = await dispatch(updateStatus(payload)).unwrap();
        toast({ title: res?.message ?? "Status updated successfully", duration: DEFAULT_TOAST_DURATION });
      } else {
        const res = await dispatch(addStatus(payload)).unwrap();
        toast({ title: res?.message ?? "Status created successfully", duration: DEFAULT_TOAST_DURATION });
      }
      setDialogOpen(false);
      dispatch(fetchStatuses({}));
    } catch (e: any) {
      toast({ title: typeof e === "string" ? e : (e?.message || "Error saving status"), variant: "destructive", duration: DEFAULT_TOAST_DURATION });
    }
  };
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      const res = await dispatch(deleteStatus(deleteId)).unwrap();
      setDeleteId(null);
      toast({ title: res?.message ?? "Status deleted successfully", duration: DEFAULT_TOAST_DURATION });
      dispatch(fetchStatuses({}));
    } catch (e: any) {
      toast({ title: typeof e === "string" ? e : (e?.message || "Error deleting status"), variant: "destructive", duration: DEFAULT_TOAST_DURATION });
    }
  };
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Status Master</h1>
          <p className="text-sm text-muted-foreground">Manage status master data</p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
          <div className="flex flex-1 flex-col sm:flex-row items-start sm:items-center gap-4 w-full sm:max-w-4xl">
            <div className="relative w-full sm:max-w-md">
              ...
              <Input placeholder="Search statuses..." value={search} onChange={(e) => { setSearch(e.target.value); setCurrentPage(1); }} className="pl-9 h-9 text-xs" />
            </div>
            ...
            {uniqueCategories.length > 0 && (
              <div className="flex items-center gap-2 shrink-0">
                ...
                <Select value={categoryFilter} onValueChange={(v) => { setCategoryFilter(v); setCurrentPage(1); }}>
                  <SelectTrigger className="w-32 h-9 text-xs"><SelectValue placeholder="All Categories" /></SelectTrigger>
                  <SelectContent>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b bg-muted/50">
                  ...
                  <th className="text-left p-3 font-semibold text-muted-foreground uppercase text-xs">Status Code</th>
                  <th className="text-left p-3 font-semibold text-muted-foreground uppercase text-xs">Status Name</th>
              ...
              <tbody>
                {paginated.map((item: any, idx: number) => {
                  ...
                  const code = item.statusCode ?? item.STATUS_CODE;
                  const name = item.statusName ?? item.STATUS_NAME;
                  const category = item.statusCategory ?? item.STATUS_CATEGORY;
                ...
                {paginated.length === 0 && (
                  <tr><td colSpan={7} className="p-8 text-center text-muted-foreground">No statuses found</td></tr>
                )}
              </tbody>
            </table>
          )}
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
          ...
          <div className="space-y-6">
            <div>
              <h3 className="text-sm font-semibold text-muted-foreground border-b pb-2 mb-3">Status Information</h3>
              <div className="grid grid-cols-2 gap-4">
                {renderField("STATUS_CODE", "Status Code", "text", undefined, true, "e.g., OPEN", 30)}
                {renderField("STATUS_NAME", "Status Name", "text", undefined, true, "e.g., Open", 100)}
              </div>
            </div>
            <div>
              <h3 className="text-sm font-semibold text-muted-foreground border-b pb-2 mb-3">Details</h3>
              <div className="grid grid-cols-2 gap-4">
                {renderField("STATUS_CATEGORY", "Category", "text", undefined, false, "e.g., PURCHASE", 50)}
      ...
      <AlertDialog open={!!deleteId} onOpenChange={(open) => !open && setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>This will permanently delete this status. Statuses already used by existing purchase requests cannot be deleted.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
