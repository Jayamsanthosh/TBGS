...
import { fetchItems, addItem, updateItem, deleteItem, clearError, type AnimalCategoryGridData } from "@/lib/animalCategoryMasterSlice";
...
export default function AnimalCategoryMasterPage() {
  const dispatch = useAppDispatch();
  const { items, loading, error } = useAppSelector((s) => s.animalCategoryMaster);
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((d: any) => {
      const searchable = [d.ANIMAL_CATEGORY_NAME, d.REMARKS].join(" ").toLowerCase();
      ...
      return true;
    }).sort((a: any, b: any) => Number(b.ANIMAL_CATEGORY_ID) - Number(a.ANIMAL_CATEGORY_ID));
  ...
  const openEdit = (item: any) => {
    setForm({
      ANIMAL_CATEGORY_NAME: item.ANIMAL_CATEGORY_NAME || "",
  ...
  const handleSave = async () => {
    if (!form.ANIMAL_CATEGORY_NAME?.trim()) {
      toast({ title: "Animal category name is required", variant: "destructive", duration: DEFAULT_TOAST_DURATION });
      return;
    }
    if (!editing && normalizeStatus(form.STATUS_MASTER) === "INACTIVE") {
      toast({ title: "New animal category cannot be set to Inactive", variant: "destructive", duration: DEFAULT_TOAST_DURATION });
      return;
    }

    const payload: any = {
      ANIMAL_CATEGORY_NAME: form.ANIMAL_CATEGORY_NAME.trim(),
    ...
    try {
      if (editing) {
        payload.ANIMAL_CATEGORY_ID = Number(editing.id);
        const res = await dispatch(updateItem(payload as AnimalCategoryGridData)).unwrap();
        toast({ title: res?.message ?? "Animal category updated successfully", duration: DEFAULT_TOAST_DURATION });
      } else {
        const res = await dispatch(addItem(payload as AnimalCategoryGridData)).unwrap();
        toast({ title: res?.message ?? "Animal category created successfully", duration: DEFAULT_TOAST_DURATION });
      }
      setDialogOpen(false);
      dispatch(fetchItems(statusFilter));
    } catch (e: any) {
      toast({ title: typeof e === 'string' ? e : (e?.payload || e?.message || "Error saving animal category"), variant: "destructive", duration: DEFAULT_TOAST_DURATION });
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setDeleteId(null);
      toast({ title: res?.message ?? "Animal category deleted successfully", duration: DEFAULT_TOAST_DURATION });
      dispatch(fetchItems(statusFilter));
    } catch (e: any) {
      toast({ title: typeof e === 'string' ? e : (e?.payload || e?.message || "Error deleting animal category"), variant: "destructive", duration: DEFAULT_TOAST_DURATION });
    }
  };
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Animal Category Master</h1>
          <p className="text-sm text-muted-foreground">Manage animal category master data</p>
        </div>
        <Button onClick={openAdd} className="bg-primary text-primary-foreground hover:bg-primary/90">
          <Plus className="w-4 h-4 mr-2" /> Add Animal Category
        </Button>
      </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
          <div className="flex items-center gap-2 w-full sm:max-w-md">
            <div className="relative flex-1">
              ...
              <Input placeholder="Search animal categories..." value={search} onChange={(e) => { setSearch(e.target.value); setCurrentPage(1); }} className="pl-9 h-9 text-xs" />
            </div>
              ...
              <tbody>
                {paginated.map((item: any, idx: number) => (
                  <tr key={item.id || idx} className="border-b hover:bg-muted/30 transition-colors">
                    ...
                    <td className="p-3 font-medium">{item.ANIMAL_CATEGORY_ID}</td>
                    <td className="p-3">{item.ANIMAL_CATEGORY_NAME}</td>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editing ? "Edit Animal Category" : "Add Animal Category"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <div className="grid grid-cols-1 gap-4">
              {renderField("ANIMAL_CATEGORY_NAME", "Name", "text", form, setForm, true, "e.g., Dangerous Game")}
      ...
      <AlertDialog open={!!deleteId} onOpenChange={(open) => !open && setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>This will permanently delete this animal category record.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
