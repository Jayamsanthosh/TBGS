...
import { fetchRackSectionMasters, addRackSectionMaster, updateRackSectionMaster, deleteRackSectionMaster, fetchRackSectionMasterById, clearRackSectionMasterError } from "@/lib/rackSectionMasterSlice";
...
export default function RackSectionMasterPage() {
  const dispatch = useAppDispatch();
  const { records, loading, error } = useAppSelector((s) => s.rackSectionMaster);
  ...
  const filtered = useMemo(() => {
    ...
    return records.filter((d: any) => {
      const searchable = [d.RACK_SECTION_NAME, d.RACK_SECTION_DESCRIPTION].join(" ").toLowerCase();
      return searchable.includes(search.toLowerCase());
    }).sort((a: any, b: any) => Number(b.id || b.RACK_SECTION_ID) - Number(a.id || a.RACK_SECTION_ID));
  }, [records, search]);
  ...
  const openEdit = async (item: any) => {
    ...
    setEditing(item);
    try {
      const response = await dispatch(fetchRackSectionMasterById(Number(item.RACK_SECTION_ID))).unwrap();
      if (response) {
        setForm({
          RACK_SECTION_NAME: response.RACK_SECTION_NAME || "",
          RACK_SECTION_DESCRIPTION: response.RACK_SECTION_DESCRIPTION || "",
  ...
  const handleSave = async () => {
    if (!form.RACK_SECTION_NAME?.trim()) {
      toast({ title: "Rack section name is required", variant: "destructive", duration: DEFAULT_TOAST_DURATION });
      return;
    }
    if (form.RACK_SECTION_NAME.trim().length > 10) {
      toast({ title: "Rack section name must be 10 characters or less", variant: "destructive", duration: DEFAULT_TOAST_DURATION });
      return;
    }

    if (!editing && normalizeStatus(form.STATUS_MASTER) === "INACTIVE") {
      toast({ title: "New rack section cannot be set to Inactive", variant: "destructive", duration: DEFAULT_TOAST_DURATION });
      return;
    }

    try {
      const payload: Record<string, any> = {
        RACK_SECTION_NAME: form.RACK_SECTION_NAME.trim(),
        RACK_SECTION_DESCRIPTION: form.RACK_SECTION_DESCRIPTION?.trim() || null,
      ...
      if (editing) {
        ...
        payload.RACK_SECTION_ID = Number(editing.id);
        const res = await dispatch(updateRackSectionMaster(payload)).unwrap();
        toast({ title: res?.message ?? "Rack section updated successfully", duration: DEFAULT_TOAST_DURATION });
      } else {
        const res = await dispatch(addRackSectionMaster(payload)).unwrap();
        toast({ title: res?.message ?? "Rack section created successfully", duration: DEFAULT_TOAST_DURATION });
      }
      setDialogOpen(false);
      dispatch(fetchRackSectionMasters());
    } catch (e: any) {
      toast({ title: typeof e === "string" ? e : (e?.message || "Error saving rack section"), variant: "destructive", duration: DEFAULT_TOAST_DURATION });
    }
  };
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      const res = await dispatch(deleteRackSectionMaster(deleteId)).unwrap();
      setDeleteId(null);
      toast({ title: res?.message ?? "Rack section deleted successfully", duration: DEFAULT_TOAST_DURATION });
      dispatch(fetchRackSectionMasters());
    } catch (e: any) {
      toast({ title: typeof e === "string" ? e : (e?.message || "Error deleting rack section"), variant: "destructive", duration: DEFAULT_TOAST_DURATION });
    }
  };
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Rack Section Master</h1>
          <p className="text-sm text-muted-foreground">Manage rack sections</p>
        </div>
        <Button onClick={openAdd} className="bg-primary text-primary-foreground hover:bg-primary/90">
          <Plus className="w-4 h-4 mr-2" /> Add Rack Section
        </Button>
      </div>
      ...
      <div className="bg-card rounded-xl border p-4 sm:p-6 shadow-sm overflow-hidden w-full">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
          <div className="relative w-full sm:max-w-md">
            ...
            <Input placeholder="Search rack sections..." value={search} onChange={(e) => { setSearch(e.target.value); setCurrentPage(1); }} className="pl-9 h-9 text-xs" />
          </div>
              ...
              <tbody>
                {paginated.map((item: any, idx: number) => (
                  <tr key={item.id || idx} className="border-b hover:bg-muted/30 transition-colors">
                    ...
                    <td className="p-3 font-medium">{item.RACK_SECTION_ID}</td>
                    <td className="p-3">{item.RACK_SECTION_NAME}</td>
                    <td className="p-3">{item.RACK_SECTION_DESCRIPTION || "-"}</td>
                    <td className="p-3">
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editing ? "Edit Rack Section" : "Add Rack Section"}</DialogTitle>
          </DialogHeader>
          {loadingEdit ? (
          ...
          <div className="space-y-6">
            <div>
              <h3 className="text-sm font-semibold text-muted-foreground border-b pb-2 mb-3">Rack Section Details</h3>
              <div className="grid grid-cols-2 gap-4">
                {renderField("RACK_SECTION_NAME", "Rack Section Name", "text", undefined, true, "e.g., SEC-A", 10)}
                {renderField("RACK_SECTION_DESCRIPTION", "Description", "text", undefined, false, "e.g., Section A rack storage")}
              </div>
            </div>
            <div>
      ...
      <AlertDialog open={!!deleteId} onOpenChange={(open) => !open && setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>This will permanently delete this rack section.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
