...
import { fetchItems, addItem, updateItem, deleteItem, clearError, type GunSourceTypeGridData } from "@/lib/gunSourceTypeMasterSlice";
...
export default function GunSourceTypeMasterPage() {
  const dispatch = useAppDispatch();
  const { items, loading, error } = useAppSelector((s) => s.gunSourceTypeMaster);
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((d: any) => {
      const searchable = [d.GUN_SOURCE_TYPE_NAME, d.GUN_SOURCE_TYPE_DESCRIPTION, d.REMARKS].join(" ").toLowerCase();
      ...
      return true;
    }).sort((a: any, b: any) => Number(b.GUN_SOURCE_TYPE_ID) - Number(a.GUN_SOURCE_TYPE_ID));
  ...
  const emptyForm = () => ({
    GUN_SOURCE_TYPE_NAME: "",
    GUN_SOURCE_TYPE_DESCRIPTION: "",
    REMARKS: "",
    STATUS_MASTER: "AC",
    USER: "Admin",
    MAC_ADDRESS: "WEB",
  ...
  const openEdit = (item: any) => {
    setForm({
      GUN_SOURCE_TYPE_NAME: item.GUN_SOURCE_TYPE_NAME || "",
      GUN_SOURCE_TYPE_DESCRIPTION: item.GUN_SOURCE_TYPE_DESCRIPTION || "",
      REMARKS: item.REMARKS || "",
      STATUS_MASTER: item.STATUS_MASTER === "ACTIVE" ? "AC" : item.STATUS_MASTER === "INACTIVE" ? "IN" : (item.STATUS_MASTER || "AC"),
      USER: "Admin",
      MAC_ADDRESS: "WEB",
    });
    setEditing(item);
  ...
  const handleSave = async () => {
    if (!form.GUN_SOURCE_TYPE_NAME?.trim()) {
      toast({ title: "Gun source type name is required", variant: "destructive", duration: DEFAULT_TOAST_DURATION });
      return;
    }
    if (!editing && normalizeStatus(form.STATUS_MASTER) === "INACTIVE") {
      toast({ title: "New gun source type cannot be set to Inactive", variant: "destructive", duration: DEFAULT_TOAST_DURATION });
      return;
    }

    const payload: any = {
      GUN_SOURCE_TYPE_NAME: form.GUN_SOURCE_TYPE_NAME.trim(),
      GUN_SOURCE_TYPE_DESCRIPTION: form.GUN_SOURCE_TYPE_DESCRIPTION?.trim() || null,
    ...
    try {
      if (editing) {
        payload.GUN_SOURCE_TYPE_ID = Number(editing.id);
        const res = await dispatch(updateItem(payload as GunSourceTypeGridData)).unwrap();
        toast({ title: res?.message ?? "Gun source type updated successfully", duration: DEFAULT_TOAST_DURATION });
      } else {
        const res = await dispatch(addItem(payload as GunSourceTypeGridData)).unwrap();
        toast({ title: res?.message ?? "Gun source type created successfully", duration: DEFAULT_TOAST_DURATION });
      }
      setDialogOpen(false);
      dispatch(fetchItems(statusFilter));
    } catch (e: any) {
      toast({ title: typeof e === 'string' ? e : (e?.payload || e?.message || "Error saving gun source type"), variant: "destructive", duration: DEFAULT_TOAST_DURATION });
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setDeleteId(null);
      toast({ title: res?.message ?? "Gun source type deleted successfully", duration: DEFAULT_TOAST_DURATION });
      dispatch(fetchItems(statusFilter));
    } catch (e: any) {
      toast({ title: typeof e === 'string' ? e : (e?.payload || e?.message || "Error deleting gun source type"), variant: "destructive", duration: DEFAULT_TOAST_DURATION });
    }
  };
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Gun Source Type Master</h1>
          <p className="text-sm text-muted-foreground">Manage gun source type master data</p>
        </div>
        <Button onClick={openAdd} className="bg-primary text-primary-foreground hover:bg-primary/90">
          <Plus className="w-4 h-4 mr-2" /> Add Gun Source Type
        </Button>
      </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
          <div className="flex items-center gap-2 w-full sm:max-w-md">
            <div className="relative flex-1">
              ...
              <Input placeholder="Search gun source types..." value={search} onChange={(e) => { setSearch(e.target.value); setCurrentPage(1); }} className="pl-9 h-9 text-xs" />
            </div>
              ...
              <tbody>
                {paginated.map((item: any, idx: number) => (
                  <tr key={item.id || idx} className="border-b hover:bg-muted/30 transition-colors">
                    ...
                    <td className="p-3 font-medium">{item.GUN_SOURCE_TYPE_ID}</td>
                    <td className="p-3">{item.GUN_SOURCE_TYPE_NAME}</td>
                    <td className="p-3">{item.GUN_SOURCE_TYPE_DESCRIPTION || "-"}</td>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editing ? "Edit Gun Source Type" : "Add Gun Source Type"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <div className="grid grid-cols-1 gap-4">
              {renderField("GUN_SOURCE_TYPE_NAME", "Name", "text", form, setForm, true, "e.g., Client")}
              {renderField("GUN_SOURCE_TYPE_DESCRIPTION", "Description", "text", form, setForm, false, "e.g., Client Provided Gun")}
      ...
      <AlertDialog open={!!deleteId} onOpenChange={(open) => !open && setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>This will permanently delete this gun source type record.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
