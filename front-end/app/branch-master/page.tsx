...
import { fetchBranches, addBranch, updateBranch, deleteBranch, clearBranchError, type BranchGridData } from "@/lib/branchMasterSlice";
...
export default function BranchMasterPage() {
  ...
  const { branches, loading, error } = useAppSelector((s) => s.branch);
  ...
  const filtered = useMemo(() => {
    ...
    return branches.filter((d: any) => {
      const searchable = [d.BRANCH_NAME, d.BRANCH_DESCRIPTION, d.REMARKS, d.STATUS_MASTER].join(" ").toLowerCase();
      ...
      return true;
    }).sort((a: any, b: any) => Number(b.BRANCH_ID) - Number(a.BRANCH_ID));
  ...
  useEffect(() => {
    dispatch(fetchBranches(statusFilter));
  ...
  const handleSave = async () => {
    if (!form.BRANCH_NAME?.trim()) {
      toast({ title: "Branch name is required", variant: "destructive", duration: DEFAULT_TOAST_DURATION });
      return;
    }
    if (form.BRANCH_NAME.trim().length > 50) {
      toast({ title: "Branch name must be 50 characters or less", variant: "destructive", duration: DEFAULT_TOAST_DURATION });
      return;
    }
    if (form.BRANCH_DESCRIPTION && form.BRANCH_DESCRIPTION.trim().length > 50) {
    ...
    if (!editing && normalizeStatus(form.STATUS_MASTER) === "INACTIVE") {
      toast({ title: "New branch cannot be set to Inactive", variant: "destructive", duration: DEFAULT_TOAST_DURATION });
      return;
    }
    ...
    try {
      const payload: Record<string, any> = {
        ...
        BRANCH_DESCRIPTION: form.BRANCH_DESCRIPTION?.trim() || null,
      ...
      if (editing) {
        payload.BRANCH_ID = Number(editing.id);
        const res = await dispatch(updateBranch(payload as BranchGridData)).unwrap();
        toast({ title: res?.message ?? "Branch updated successfully", duration: DEFAULT_TOAST_DURATION });
      } else {
        const res = await dispatch(addBranch(payload as BranchGridData)).unwrap();
        toast({ title: res?.message ?? "Branch created successfully", duration: DEFAULT_TOAST_DURATION });
      }
      setDialogOpen(false);
      dispatch(fetchBranches(statusFilter));
    } catch (e: any) {
      toast({ title: typeof e === "string" ? e : (e?.message || "Error saving branch"), variant: "destructive", duration: DEFAULT_TOAST_DURATION });
    }
  };
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      const res = await dispatch(deleteBranch(deleteId)).unwrap();
      setDeleteId(null);
      toast({ title: res?.message ?? "Branch deleted successfully", duration: DEFAULT_TOAST_DURATION });
      dispatch(fetchBranches(statusFilter));
    } catch (e: any) {
      toast({ title: typeof e === "string" ? e : (e?.message || "Error deleting branch"), variant: "destructive", duration: DEFAULT_TOAST_DURATION });
    }
  };
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Branch Master</h1>
          <p className="text-sm text-muted-foreground">Manage branch master data</p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
          <div className="flex items-center gap-2 w-full sm:max-w-md">
            <div className="relative flex-1">
              ...
              <Input placeholder="Search branches..." value={search} onChange={(e) => { setSearch(e.target.value); setCurrentPage(1); }} className="pl-9 h-9 text-xs" />
            </div>
              ...
              <tbody>
                {paginated.map((item: any, idx: number) => (
                  <tr key={item.id || idx} className="border-b hover:bg-muted/30 transition-colors">
                    ...
                    <td className="p-3 font-medium">{item.BRANCH_ID}</td>
                    <td className="p-3">{item.BRANCH_NAME}</td>
                    <td className="p-3">{item.BRANCH_DESCRIPTION || "-"}</td>
        ...
        <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-2 gap-4">
              {renderField("BRANCH_NAME", "Branch Name", "text", undefined, true, "e.g., Arusha Head Office", 50)}
              {renderField("BRANCH_DESCRIPTION", "Description", "text", undefined, false, "e.g., Main office", 50)}
