...
import { fetchGunBrands, fetchGunBrandById, addGunBrand, updateGunBrand, deleteGunBrand, clearGunBrandError, type GunBrandGridData } from "@/lib/gunBrandMasterSlice";
...
export default function GunBrandMasterPage() {
  const dispatch = useAppDispatch();
  const { gunBrands, loading, error } = useAppSelector((s) => s.gunBrand);
  ...
  const [businessPartners, setBusinessPartners] = useState<{ BP_ID: number; BP_NAME: string }[]>([]);
  ...
  const filtered = useMemo(() => {
    if (!Array.isArray(gunBrands)) return [];
    return gunBrands.filter((d: any) => {
      const searchable = [d.BRAND_NAME, d.Country_Name, d.CONTACT_PERSON, d.EMAIL, d.REMARKS].join(" ").toLowerCase();
      return searchable.includes(search.toLowerCase());
    }).sort((a: any, b: any) => Number(b.GUN_BRAND_ID) - Number(a.GUN_BRAND_ID));
  }, [gunBrands, search]);
  ...
  useEffect(() => {
    dispatch(fetchGunBrands(statusFilter));
  }, [dispatch, statusFilter]);

  useEffect(() => {
    fetch(`${API_URL}/country-master`)
      .then(r => r.json())
      .then(d => setCountries(d.data || []))
      .catch(() => {});
    fetch(`${API_URL}/business-partner-master`)
      .then(r => r.json())
      .then(d => setBusinessPartners(d.data || []))
      .catch(() => {});
  }, []);
  ...
  const openEdit = async (item: any) => {
    setEditing(item);
    const detail = item && item.GUN_BRAND_ID != null
      ? await dispatch(fetchGunBrandById(Number(item.GUN_BRAND_ID))).unwrap().catch(() => null)
      : null;
    const src: any = detail ? detail : item;
    setForm({
      GUN_BRAND_ID: src.GUN_BRAND_ID || 0,
      BRAND_NAME: src.BRAND_NAME || "",
      BP_ID: String(src.BP_ID || ""),
      COUNTRY_OF_ORIGIN: String(src.COUNTRY_OF_ORIGIN || ""),
      WEBSITE: src.WEBSITE || "",
      CONTACT_PERSON: src.CONTACT_PERSON || "",
      CONTACT_NUMBER: formatTanzaniaPhone(src.CONTACT_NUMBER),
      EMAIL: src.EMAIL || "",
      WARRANTY_AVAILABLE: src.WARRANTY_AVAILABLE || "",
      REMARKS: src.REMARKS || "",
      STATUS_MASTER: src.STATUS_MASTER === "ACTIVE" ? "AC" : (src.STATUS_MASTER === "INACTIVE" ? "IA" : (src.STATUS_MASTER || "AC")),
      USER: "Admin",
      MAC_ADDRESS: "WEB",
    });
    setDialogOpen(true);
  ...
  const handleSave = async () => {
    ...
    if (!editing && normalizeStatus(form.STATUS_MASTER) !== "ACTIVE") {
      toast({ title: "New gun brand cannot be set to Inactive", variant: "destructive", duration: DEFAULT_TOAST_DURATION });
      return;
    }
    ...
    try {
      const payload: Record<string, any> = {
        ...
        BP_ID: form.BP_ID ? Number(form.BP_ID) : null,
        ...
        WARRANTY_AVAILABLE: form.WARRANTY_AVAILABLE || null,
      ...
      if (editing) {
        payload.GUN_BRAND_ID = Number(editing.id);
        const res = await dispatch(updateGunBrand(payload as GunBrandGridData)).unwrap();
        toast({ title: res?.message ?? "Gun brand updated successfully", duration: DEFAULT_TOAST_DURATION });
      } else {
        const res = await dispatch(addGunBrand(payload as GunBrandGridData)).unwrap();
        toast({ title: res?.message ?? "Gun brand created successfully", duration: DEFAULT_TOAST_DURATION });
      }
      setDialogOpen(false);
      dispatch(fetchGunBrands(statusFilter));
    } catch (e: any) {
      toast({ title: typeof e === 'string' ? e : (e?.payload || e?.message || "Error saving gun brand"), variant: "destructive", duration: DEFAULT_TOAST_DURATION });
    }
  };
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      const res = await dispatch(deleteGunBrand(deleteId)).unwrap();
      setDeleteId(null);
      toast({ title: res?.message ?? "Gun brand deleted successfully", duration: DEFAULT_TOAST_DURATION });
      dispatch(fetchGunBrands(statusFilter));
    } catch (e: any) {
      toast({ title: typeof e === 'string' ? e : (e?.payload || e?.message || "Error deleting gun brand"), variant: "destructive", duration: DEFAULT_TOAST_DURATION });
    }
  };
      ...
      <div key={key} className={type === "textarea" ? "col-span-2" : baseClass}>
        ...
        {type === "select" ? (
          ...
          <Input
            ...
            value={form[key] ?? ""}
            onChange={(e) => setForm({ ...form, [key]: formatter ? formatter(e.target.value) : (type === "number" ? (e.target.value ? Number(e.target.value) : null) : e.target.value) })}
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Gun Brand Master</h1>
          <p className="text-sm text-muted-foreground">Manage gun brand master data</p>
        </div>
        <Button onClick={openAdd} className="bg-primary text-primary-foreground hover:bg-primary/90">
          <Plus className="w-4 h-4 mr-2" /> Add Gun Brand
        </Button>
      </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
          <div className="flex items-center gap-2 w-full sm:max-w-md">
            <div className="relative flex-1">
              ...
              <Input placeholder="Search gun brands..." value={search} onChange={(e) => { setSearch(e.target.value); setCurrentPage(1); }} className="pl-9 h-9 text-xs" />
            </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b bg-muted/50">
                  ...
                  <th className="text-left p-3 font-semibold text-muted-foreground uppercase text-xs">Brand Name</th>
              ...
              <tbody>
                {paginated.map((item: any, idx: number) => (
                  <tr key={item.id || idx} className="border-b hover:bg-muted/30 transition-colors">
                    ...
                    <td className="p-3 font-medium">{item.GUN_BRAND_ID}</td>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editing ? "Edit Gun Brand" : "Add Gun Brand"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-6">
            <div className="grid grid-cols-2 gap-4">
              ...
              {renderField("BP_ID", "Business Partner", "select", businessPartners.map(bp => ({ value: String(bp.BP_ID), label: bp.BP_NAME })), false, "Select BP")}
              {renderField("COUNTRY_OF_ORIGIN", "Country of Origin", "select", countries.map(c => ({ value: String(c.Country_Id), label: c.Country_Name })), false, "Select Country")}
              {renderField("WEBSITE", "Website", "text", undefined, false, "e.g., https://example.com")}
              {renderField("CONTACT_PERSON", "Contact Person", "text", undefined, false, "Enter contact person")}
              {renderField("CONTACT_NUMBER", "Contact Number", "text", undefined, false, "e.g., +255 700 000 000", undefined, formatTanzaniaPhone)}
              {renderField("EMAIL", "Email", "text", undefined, false, "Enter email address")}
              {renderField("WARRANTY_AVAILABLE", "Warranty Available", "select", WARRANTY_OPTIONS, false, "Select Yes/No")}
            </div>
      ...
      <AlertDialog open={!!deleteId} onOpenChange={(open) => !open && setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>This will permanently delete this gun brand.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
