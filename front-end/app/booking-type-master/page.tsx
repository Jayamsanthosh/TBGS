...
import { fetchBookingTypes, addBookingType, updateBookingType, deleteBookingType, clearBookingTypeError, type BookingTypeGridData } from "@/lib/bookingTypeMasterSlice";
...
export default function BookingTypeMasterPage() {
  const dispatch = useAppDispatch();
  const { bookingTypes, loading, error } = useAppSelector((s) => s.bookingType);
  ...
  const filtered = useMemo(() => {
    if (!Array.isArray(bookingTypes)) return [];
    return bookingTypes.filter((d: any) => {
      const searchable = [d.BOOKING_TYPE_NAME, d.BOOKING_TYPE_DESCRIPTION, d.REMARKS, d.STATUS_MASTER].join(" ").toLowerCase();
      ...
      return true;
    }).sort((a: any, b: any) => Number(b.BOOKING_TYPE_ID) - Number(a.BOOKING_TYPE_ID));
  }, [bookingTypes, search, statusFilter]);
  ...
  const openEdit = (item: any) => {
    setEditing(item);
    setForm({
      BOOKING_TYPE_ID: item.BOOKING_TYPE_ID || 0,
      BOOKING_TYPE_NAME: item.BOOKING_TYPE_NAME || "",
      BOOKING_TYPE_DESCRIPTION: item.BOOKING_TYPE_DESCRIPTION || "",
  ...
  const handleSave = async () => {
    if (!form.BOOKING_TYPE_NAME?.trim()) {
      toast({ title: "Booking type name is required", variant: "destructive", duration: DEFAULT_TOAST_DURATION });
      return;
    }
    if (form.BOOKING_TYPE_NAME.trim().length > 50) {
      toast({ title: "Booking type name must be 50 characters or less", variant: "destructive", duration: DEFAULT_TOAST_DURATION });
      return;
    }
    if (form.BOOKING_TYPE_DESCRIPTION && form.BOOKING_TYPE_DESCRIPTION.trim().length > 50) {
    ...
    if (!editing && normalizeStatus(form.STATUS_MASTER) === "INACTIVE") {
      toast({ title: "New booking type cannot be set to Inactive", variant: "destructive", duration: DEFAULT_TOAST_DURATION });
      return;
    }

    try {
      const payload: Record<string, any> = {
        BOOKING_TYPE_NAME: form.BOOKING_TYPE_NAME.trim(),
        BOOKING_TYPE_DESCRIPTION: form.BOOKING_TYPE_DESCRIPTION?.trim() || null,
      ...
      if (editing) {
        payload.BOOKING_TYPE_ID = Number(editing.id);
        const res = await dispatch(updateBookingType(payload as BookingTypeGridData)).unwrap();
        toast({ title: res?.message ?? "Booking type updated successfully", duration: DEFAULT_TOAST_DURATION });
      } else {
        const res = await dispatch(addBookingType(payload as BookingTypeGridData)).unwrap();
        toast({ title: res?.message ?? "Booking type created successfully", duration: DEFAULT_TOAST_DURATION });
      }
      setDialogOpen(false);
      dispatch(fetchBookingTypes(statusFilter));
    } catch (e: any) {
      toast({ title: typeof e === "string" ? e : (e?.message || "Error saving booking type"), variant: "destructive", duration: DEFAULT_TOAST_DURATION });
    }
  };
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      const res = await dispatch(deleteBookingType(deleteId)).unwrap();
      setDeleteId(null);
      toast({ title: res?.message ?? "Booking type deleted successfully", duration: DEFAULT_TOAST_DURATION });
      dispatch(fetchBookingTypes(statusFilter));
    } catch (e: any) {
      toast({ title: typeof e === "string" ? e : (e?.message || "Error deleting booking type"), variant: "destructive", duration: DEFAULT_TOAST_DURATION });
    }
  };
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Booking Type Master</h1>
          <p className="text-sm text-muted-foreground">Manage booking type master data</p>
        </div>
        <Button onClick={openAdd} className="bg-primary text-primary-foreground hover:bg-primary/90">
          <Plus className="w-4 h-4 mr-2" /> Add Booking Type
        </Button>
      </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
          <div className="flex items-center gap-2 w-full sm:max-w-md">
            <div className="relative flex-1">
              ...
              <Input placeholder="Search booking types..." value={search} onChange={(e) => { setSearch(e.target.value); setCurrentPage(1); }} className="pl-9 h-9 text-xs" />
            </div>
              ...
              <tbody>
                {paginated.map((item: any, idx: number) => (
                  <tr key={item.id || idx} className="border-b hover:bg-muted/30 transition-colors">
                    ...
                    <td className="p-3 font-medium">{item.BOOKING_TYPE_ID}</td>
                    <td className="p-3">{item.BOOKING_TYPE_NAME}</td>
                    <td className="p-3">{item.BOOKING_TYPE_DESCRIPTION || "-"}</td>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editing ? "Edit Booking Type" : "Add Booking Type"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-6">
            <div className="grid grid-cols-2 gap-4">
              {renderField("BOOKING_TYPE_NAME", "Booking Type Name", "text", undefined, true, "e.g., Hunting", 50)}
              {renderField("BOOKING_TYPE_DESCRIPTION", "Description", "text", undefined, false, "e.g., Big game hunting", 50)}
      ...
      <AlertDialog open={!!deleteId} onOpenChange={(open) => !open && setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>This will permanently delete this booking type.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
