...
import { fetchSalesTypeMasters, addSalesTypeMaster, updateSalesTypeMaster, deleteSalesTypeMaster, fetchSalesTypeMasterById, clearSalesTypeMasterError } from "@/lib/salesTypeMasterSlice";
...
export default function SalesTypeMasterPage() {
  const dispatch = useAppDispatch();
  const { records, loading, error } = useAppSelector((s) => s.salesTypeMaster);
  ...
  const filtered = useMemo(() => {
    ...
    return records.filter((d: any) => {
      const searchable = [d.SALES_TYPE_NAME, d.SALES_TYPE_DESCRIPTION].join(" ").toLowerCase();
      return searchable.includes(search.toLowerCase());
    }).sort((a: any, b: any) => Number(b.SALES_TYPE_ID) - Number(a.SALES_TYPE_ID));
  }, [records, search]);
  ...
  const openEdit = async (item: any) => {
    ...
    setEditing(item);
    try {
      const response = await dispatch(fetchSalesTypeMasterById(Number(item.SALES_TYPE_ID))).unwrap();
      if (response) {
        setForm({
          SALES_TYPE_NAME: response.SALES_TYPE_NAME || "",
          SALES_TYPE_DESCRIPTION: response.SALES_TYPE_DESCRIPTION || "",
  ...
  const handleSave = async () => {
    ...
    if (!form.SALES_TYPE_NAME?.trim()) {
      toast({ title: "Sales type name is required", variant: "destructive", duration: DEFAULT_TOAST_DURATION });
      return;
    }

    try {
      const payload: Record<string, any> = {
        SALES_TYPE_NAME: form.SALES_TYPE_NAME.trim(),
        SALES_TYPE_DESCRIPTION: form.SALES_TYPE_DESCRIPTION?.trim() || null,
      ...
      if (editing) {
        ...
        payload.SALES_TYPE_ID = Number(editing.id);
        const res = await dispatch(updateSalesTypeMaster(payload)).unwrap();
        toast({ title: res?.message ?? "Sales type updated successfully", duration: DEFAULT_TOAST_DURATION });
      } else {
        const res = await dispatch(addSalesTypeMaster(payload)).unwrap();
        toast({ title: res?.message ?? "Sales type created successfully", duration: DEFAULT_TOAST_DURATION });
      }
      setDialogOpen(false);
      dispatch(fetchSalesTypeMasters());
    } catch (e: any) {
      toast({ title: typeof e === "string" ? e : (e?.message || "Error saving sales type"), variant: "destructive", duration: DEFAULT_TOAST_DURATION });
    }
  };
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      const res = await dispatch(deleteSalesTypeMaster(deleteId)).unwrap();
      setDeleteId(null);
      toast({ title: res?.message ?? "Sales type deleted successfully", duration: DEFAULT_TOAST_DURATION });
      dispatch(fetchSalesTypeMasters());
    } catch (e: any) {
      toast({ title: typeof e === "string" ? e : (e?.message || "Error deleting sales type"), variant: "destructive", duration: DEFAULT_TOAST_DURATION });
    }
  };
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Sales Type Master</h1>
          <p className="text-sm text-muted-foreground">Manage sales types</p>
        </div>
        <Button onClick={openAdd} className="bg-primary text-primary-foreground hover:bg-primary/90">
          <Plus className="w-4 h-4 mr-2" /> Add Sales Type
        </Button>
      </div>
      ...
      <div className="bg-card rounded-xl border p-4 sm:p-6 shadow-sm overflow-hidden w-full">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
          <div className="relative w-full sm:max-w-md">
            ...
            <Input placeholder="Search sales types..." value={search} onChange={(e) => { setSearch(e.target.value); setCurrentPage(1); }} className="pl-9 h-9 text-xs" />
          </div>
              ...
              <tbody>
                {paginated.map((item: any, idx: number) => (
                  <tr key={item.id || idx} className="border-b hover:bg-muted/30 transition-colors">
                    ...
                    <td className="p-3 font-medium">{item.SALES_TYPE_ID}</td>
                    <td className="p-3">{item.SALES_TYPE_NAME}</td>
                    <td className="p-3">{item.SALES_TYPE_DESCRIPTION || "-"}</td>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editing ? "Edit Sales Type" : "Add Sales Type"}</DialogTitle>
          </DialogHeader>
          {loadingEdit ? (
          ...
          <div className="space-y-6">
            <div>
              <h3 className="text-sm font-semibold text-muted-foreground border-b pb-2 mb-3">Sales Type Details</h3>
              <div className="grid grid-cols-2 gap-4">
                {renderField("SALES_TYPE_NAME", "Sales Type Name", "text", undefined, true, "e.g., Wholesale")}
                {renderField("SALES_TYPE_DESCRIPTION", "Description", "text", undefined, false, "e.g., Bulk order sales")}
              </div>
            </div>
            <div>
      ...
      <AlertDialog open={!!deleteId} onOpenChange={(open) => !open && setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>This will permanently delete this sales type.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
