...
import { fetchMassagers, addMassager, updateMassager, deleteMassager, clearMassagerMasterError, type MassagerMasterGridData } from "@/lib/massagerMasterSlice";
...
export default function MassagerMasterPage() {
  const dispatch = useAppDispatch();
  const { massagers, loading, error } = useAppSelector((s) => s.massagerMaster);
  ...
  useEffect(() => {
    dispatch(fetchMassagers(statusFilter));
  }, [dispatch, statusFilter]);

  useEffect(() => {
    if (error) {
      toast({ variant: 'destructive', title: 'Error', description: error, duration: DEFAULT_TOAST_DURATION });
      dispatch(clearMassagerMasterError());
    }
  }, [error, dispatch, toast]);

  const filtered = useMemo(() => {
    if (!Array.isArray(massagers)) return [];
    return massagers.filter((d: any) => {
      const searchable = [d.MASSAGER_NAME, d.ADDRESS, d.LOCATION_NAME, d.REMARKS].join(" ").toLowerCase();
      return searchable.includes(search.toLowerCase());
    }).sort((a: any, b: any) => Number(b.MASSAGER_ID || 0) - Number(a.MASSAGER_ID || 0));
  }, [massagers, search]);
  ...
  const openEdit = (item: any) => {
    setEditing(item);
    setForm({
      MASSAGER_NAME: item.MASSAGER_NAME || "",
  ...
  const handleSave = async () => {
    ...
    if (!form.MASSAGER_NAME?.trim()) {
      toast({ title: "Massager name is required", variant: "destructive", duration: DEFAULT_TOAST_DURATION });
      return;
    }

    try {
      const payload: Record<string, any> = {
        MASSAGER_NAME: form.MASSAGER_NAME.trim(),
      ...
      if (editing) {
        payload.MASSAGER_ID = Number(editing.id);
        const res = await dispatch(updateMassager(payload as MassagerMasterGridData)).unwrap();
        toast({ title: res?.message ?? "Massager updated successfully", duration: DEFAULT_TOAST_DURATION });
      } else {
        const res = await dispatch(addMassager(payload as MassagerMasterGridData)).unwrap();
        toast({ title: res?.message ?? "Massager created successfully", duration: DEFAULT_TOAST_DURATION });
      }
      setDialogOpen(false);
      dispatch(fetchMassagers(statusFilter));
    } catch (e: any) {
      toast({ title: typeof e === 'string' ? e : (e?.payload || e?.message || "Error saving massager"), variant: "destructive", duration: DEFAULT_TOAST_DURATION });
    }
  };
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      const res = await dispatch(deleteMassager(deleteId)).unwrap();
      setDeleteId(null);
      toast({ title: res?.message ?? "Massager deleted successfully", duration: DEFAULT_TOAST_DURATION });
      dispatch(fetchMassagers(statusFilter));
    } catch (e: any) {
      toast({ title: typeof e === 'string' ? e : (e?.payload || e?.message || "Error deleting massager"), variant: "destructive", duration: DEFAULT_TOAST_DURATION });
    }
  };
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Massager Master</h1>
          <p className="text-sm text-muted-foreground">Manage massager master data</p>
        </div>
        <Button onClick={openAdd} className="bg-primary text-primary-foreground hover:bg-primary/90">
          <Plus className="w-4 h-4 mr-2" /> Add Massager
        </Button>
      </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
          <div className="flex items-center gap-2 w-full sm:max-w-md">
            <div className="relative flex-1">
              ...
              <Input placeholder="Search massagers..." value={search} onChange={(e) => { setSearch(e.target.value); setCurrentPage(1); }} className="pl-9 h-9 text-xs" />
            </div>
              ...
              <tbody>
                {paginated.map((item: any, idx: number) => (
                  <tr key={item.id || idx} className="border-b hover:bg-muted/30 transition-colors">
                    ...
                    <td className="p-3 font-medium">{item.MASSAGER_ID}</td>
                    <td className="p-3">{item.MASSAGER_NAME}</td>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editing ? "Edit Massager" : "Add Massager"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-6">
            <div className="grid grid-cols-2 gap-4">
              <div className="col-span-2">
                <Label className="text-xs">Massager Name <span className="text-destructive">*</span></Label>
                <Input value={form.MASSAGER_NAME || ""} onChange={(e) => setForm({ ...form, MASSAGER_NAME: e.target.value })} placeholder="e.g., John Doe" className={`h-9 text-xs ${!form.MASSAGER_NAME?.trim() ? "border-destructive ring-1 ring-destructive/30" : ""}`} />
              </div>
              <div className="col-span-2">
      ...
      <AlertDialog open={!!deleteId} onOpenChange={(open) => !open && setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>This will permanently delete this massager record.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
