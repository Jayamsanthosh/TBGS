...
import { fetchItems, addItem, updateItem, deleteItem, clearError, type HotelRoomTypeGridData } from "@/lib/hotelRoomTypeMasterSlice";
  ...
  <div key={key} className={type === "textarea" ? "col-span-full flex flex-col gap-1.5" : "flex flex-col gap-1.5"}>
    ...
    {type === "textarea" ? (
      ...
      <Input
        ...
        onChange={(e) => setForm({ ...form, [key]: e.target.value })}
        placeholder={placeholder}
        maxLength={key === "ROOM_TYPE_NAME" ? 100 : key === "DESCRIPTION" ? 500 : undefined}
...
export default function HotelRoomTypeMasterPage() {
  const dispatch = useAppDispatch();
  const { items, loading, error } = useAppSelector((s) => s.hotelRoomTypeMaster);
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((d: any) => {
      const searchable = [d.ROOM_TYPE_NAME, d.DESCRIPTION, d.REMARKS].join(" ").toLowerCase();
      ...
      return true;
    }).sort((a: any, b: any) => Number(b.ROOM_TYPE_ID) - Number(a.ROOM_TYPE_ID));
  ...
  const handleSave = async () => {
    if (!form.ROOM_TYPE_NAME?.trim()) {
      toast({ title: "Room type name is required", variant: "destructive", duration: DEFAULT_TOAST_DURATION });
      return;
    }
    if (!editing && normalizeStatus(form.STATUS_MASTER) === "INACTIVE") {
      toast({ title: "New room type cannot be set to Inactive", variant: "destructive", duration: DEFAULT_TOAST_DURATION });
      return;
    }
    ...
    try {
      if (editing) {
        payload.ROOM_TYPE_ID = Number(editing.id);
        const res = await dispatch(updateItem(payload as HotelRoomTypeGridData)).unwrap();
        toast({ title: res?.message ?? "Room type updated successfully", duration: DEFAULT_TOAST_DURATION });
      } else {
        const res = await dispatch(addItem(payload as HotelRoomTypeGridData)).unwrap();
        toast({ title: res?.message ?? "Room type created successfully", duration: DEFAULT_TOAST_DURATION });
      }
      setDialogOpen(false);
      dispatch(fetchItems(statusFilter));
    } catch (e: any) {
      toast({ title: typeof e === 'string' ? e : (e?.payload || e?.message || "Error saving room type"), variant: "destructive", duration: DEFAULT_TOAST_DURATION });
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setDeleteId(null);
      toast({ title: res?.message ?? "Room type deleted successfully", duration: DEFAULT_TOAST_DURATION });
      dispatch(fetchItems(statusFilter));
    } catch (e: any) {
      toast({ title: typeof e === 'string' ? e : (e?.payload || e?.message || "Error deleting room type"), variant: "destructive", duration: DEFAULT_TOAST_DURATION });
    }
  };
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Hotel Room Type Master</h1>
          <p className="text-sm text-muted-foreground">Manage hotel room type master data</p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
          <div className="flex items-center gap-2 w-full sm:max-w-md">
            <div className="relative flex-1">
              ...
              <Input placeholder="Search room types..." value={search} onChange={(e) => { setSearch(e.target.value); setCurrentPage(1); }} className="pl-9 h-9 text-xs" />
            </div>
              ...
              <tbody>
                {paginated.map((item: any, idx: number) => (
                  <tr key={item.id || idx} className="border-b hover:bg-muted/30 transition-colors">
                    ...
                    <td className="p-3 font-medium">{item.ROOM_TYPE_ID}</td>
                    <td className="p-3">{item.ROOM_TYPE_NAME}</td>
        ...
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 gap-4">
              {renderField("ROOM_TYPE_NAME", "Name", "text", form, setForm, true, "e.g., Deluxe Room")}
              {renderField("DESCRIPTION", "Description", "text", form, setForm, false, "e.g., Deluxe room with king bed")}
      ...
      <AlertDialog open={!!deleteId} onOpenChange={(open) => !open && setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>This will permanently delete this room type record.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
