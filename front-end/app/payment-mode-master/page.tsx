...
import { fetchPaymentModes, addPaymentMode, updatePaymentMode, deletePaymentMode, clearPaymentModesError, type PaymentModeGridData } from "@/lib/paymentModeMasterSlice";
...
export default function PaymentModeMasterPage() {
  ...
  useEffect(() => {
    dispatch(fetchPaymentModes(statusFilter));
  ...
  const filtered = useMemo(() => {
    ...
    return paymentModes.filter((d: any) => {
      const searchable = [d.PAYMENT_MODE_NAME, String(d.PAYMENT_MODE_PERCENTAGE ?? ""), d.REMARKS].join(" ").toLowerCase();
      return searchable.includes(search.toLowerCase());
    }).sort((a: any, b: any) => Number(b.PAYMENT_MODE_ID || 0) - Number(a.PAYMENT_MODE_ID || 0));
  }, [paymentModes, search]);
  ...
  const openEdit = (item: any) => {
    ...
    setForm({
      ...
      PAYMENT_MODE_PERCENTAGE: item.PAYMENT_MODE_PERCENTAGE ?? "",
      REMARKS: item.REMARKS || "",
      STATUS_ENTRY: item.STATUS_ENTRY || "AC",
      USER: "Admin",
      MAC_ADDRESS: "WEB",
    });
    setDialogOpen(true);
  ...
  const handleSave = async () => {
    if (!editing && normalizeStatus(form.STATUS_ENTRY) !== "ACTIVE") {
    ...
    if (!form.PAYMENT_MODE_NAME?.trim()) {
      toast({ title: "Payment mode name is required", variant: "destructive", duration: DEFAULT_TOAST_DURATION });
      return;
    }
    ...
    try {
      const payload: Record<string, any> = {
        ...
        PAYMENT_MODE_PERCENTAGE: form.PAYMENT_MODE_PERCENTAGE === "" ? null : Math.max(0, Number(form.PAYMENT_MODE_PERCENTAGE) || 0),
        REMARKS: form.REMARKS?.trim() || null,
        STATUS_ENTRY: form.STATUS_ENTRY,
        USER: "Admin",
        MAC_ADDRESS: "WEB",
      };

      if (editing) {
        payload.PAYMENT_MODE_ID = Number(editing.id);
        const res = await dispatch(updatePaymentMode(payload as PaymentModeGridData)).unwrap();
        toast({ title: res?.message ?? "Payment mode updated successfully", duration: DEFAULT_TOAST_DURATION });
      } else {
        const res = await dispatch(addPaymentMode(payload as PaymentModeGridData)).unwrap();
        toast({ title: res?.message ?? "Payment mode created successfully", duration: DEFAULT_TOAST_DURATION });
      }
      setDialogOpen(false);
      dispatch(fetchPaymentModes(statusFilter));
    } catch (e: any) {
      toast({ title: typeof e === 'string' ? e : (e?.payload || e?.message || "Error saving payment mode"), variant: "destructive", duration: DEFAULT_TOAST_DURATION });
    }
  };
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      const res = await dispatch(deletePaymentMode(deleteId)).unwrap();
      setDeleteId(null);
      toast({ title: res?.message ?? "Payment mode deleted successfully", duration: DEFAULT_TOAST_DURATION });
      dispatch(fetchPaymentModes(statusFilter));
    } catch (e: any) {
      toast({ title: typeof e === 'string' ? e : (e?.payload || e?.message || "Error deleting payment mode"), variant: "destructive", duration: DEFAULT_TOAST_DURATION });
    }
  };
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Payment Mode Master</h1>
          <p className="text-sm text-muted-foreground">Manage payment mode master data</p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
          <div className="flex items-center gap-2 w-full sm:max-w-md">
            <div className="relative flex-1">
              ...
              <Input placeholder="Search payment modes..." value={search} onChange={(e) => { setSearch(e.target.value); setCurrentPage(1); }} className="pl-9 h-9 text-xs" />
            </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b bg-muted/50">
                  ...
                  <th className="text-left p-3 font-semibold text-muted-foreground uppercase text-xs">Percentage</th>
              ...
              <tbody>
                {paginated.map((item: any, idx: number) => (
                  <tr key={item.id || idx} className="border-b hover:bg-muted/30 transition-colors">
                    ...
                    <td className="p-3 font-medium">{item.PAYMENT_MODE_ID}</td>
                    <td className="p-3">{item.PAYMENT_MODE_NAME}</td>
                    <td className="p-3">{item.PAYMENT_MODE_PERCENTAGE != null ? item.PAYMENT_MODE_PERCENTAGE : "-"}</td>
                    <td className="p-3">{item.REMARKS || "-"}</td>
                    <td className="p-3">
                      <Badge variant="outline" className={`${normalizeStatus(item.STATUS_ENTRY) === "ACTIVE" ? "bg-green-500/10 text-green-600 border-green-200" : "bg-red-500/10 text-red-600 border-red-200"} px-2 py-0.5 text-[10px] uppercase font-bold`}>
                        {normalizeStatus(item.STATUS_ENTRY) === "ACTIVE" ? "Active" : "Inactive"}
                      </Badge>
                    </td>
                  </tr>
                ))}
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-2 gap-4">
              <div className="col-span-2">
                <Label className="text-xs">Payment Mode Name <span className="text-destructive">*</span></Label>
                <Input value={form.PAYMENT_MODE_NAME || ""} onChange={(e) => setForm({ ...form, PAYMENT_MODE_NAME: e.target.value })} placeholder="e.g., Credit Card" className={`h-9 text-xs ${!form.PAYMENT_MODE_NAME?.trim() ? "border-destructive ring-1 ring-destructive/30" : ""}`} />
              </div>
              <div className="col-span-2">
                <Label className="text-xs">Percentage</Label>
                <Input type="number" min="0" step="any" value={form.PAYMENT_MODE_PERCENTAGE ?? ""} onChange={(e) => setForm({ ...form, PAYMENT_MODE_PERCENTAGE: clampNonNegative(e.target.value) })} placeholder="e.g., 2.50" className="h-9 text-xs" />
              </div>
              <div className="col-span-2">
                <Label className="text-xs">Status</Label>
                <Select value={form.STATUS_ENTRY || "AC"} onValueChange={(v) => setForm({ ...form, STATUS_ENTRY: v })}>
      ...
      <AlertDialog open={!!deleteId} onOpenChange={(open) => !open && setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>This will permanently delete this payment mode record.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
