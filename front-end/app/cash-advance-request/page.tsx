...
export default function CashAdvanceRequestPage() {
  ...
  const baseFields: MasterField[] = [
    ...
    { key: "GROSS_PAY", label: "Gross Pay", type: "number", disabled: true, placeholder: "Auto from employee" },
    ...
    { key: "ELIGIBLE_AMOUNT", label: "Eligible Amount", type: "number", disabled: true, placeholder: "Auto from employee" },
    { key: "REQUEST_AMOUNT", label: "Request Amount", type: "number", required: true, placeholder: "0.00", disabledOnEdit: true, validate: (value: any, form: any) => {
  ...
  const handleFieldChange = useCallback((key: string, value: any, setForm: any, form: Record<string, any>): boolean => {
          ...
          .then(json => {
            if (json.success && json.data) {
              ...
              setForm((prev: any) => ({
                ...
                ELIGIBLE_AMOUNT: fullEmp.BASIC_SALARY ? Number(fullEmp.BASIC_SALARY) : prev.ELIGIBLE_AMOUNT || "",
    ...
    if (key === "DEDUCTION_FROM_DATE" || key === "DEDUCTION_TO_DATE") {
      ...
      const noOfMonths = computeDeductionMonths(fromDate, toDate);
      const amt = Number(form.REQUEST_AMOUNT);
      const months = Number(noOfMonths);
      setForm({
        ...form,
        [key]: value,
        NO_OF_MONTHS: noOfMonths,
        MONTHLY_DEDUCTION: amt > 0 && months > 0 ? Number((amt / months).toFixed(2)) : "",
      });
      return true;
    }
    ...
    if (key === "REQUEST_AMOUNT" || key === "NO_OF_MONTHS") {
      ...
      if (key === "REQUEST_AMOUNT") {
        const approved = form.APPROVED_AMOUNT;
        const prevReq = Number(form.REQUEST_AMOUNT);
        const untouched = approved == null || approved === "" || Number(approved) === prevReq;
        if (untouched) fieldUpdate.APPROVED_AMOUNT = value;
      }
      setForm(fieldUpdate);
      return true;
    }
  ...
  const storeOverrides = useMemo(() => ({
    ...
    add: async (item: CashAdvanceRequestGridData) => {
      ...
      const noOfMonths = item.NO_OF_MONTHS != null && String(item.NO_OF_MONTHS).trim() !== ""
        ? String(item.NO_OF_MONTHS)
        : computeDeductionMonths(item.DEDUCTION_FROM_DATE, item.DEDUCTION_TO_DATE);
      const monthsNum = Number(noOfMonths);
      const requestAmt = Number(item.REQUEST_AMOUNT);
      const payload: Record<string, any> = {
        ...
        NO_OF_MONTHS: noOfMonths,
        MONTHLY_DEDUCTION: item.MONTHLY_DEDUCTION != null && String(item.MONTHLY_DEDUCTION).trim() !== ""
          ? item.MONTHLY_DEDUCTION
          : (requestAmt > 0 && monthsNum > 0 ? Number((requestAmt / monthsNum).toFixed(2)) : ""),
