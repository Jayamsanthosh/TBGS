...
import {
  fetchShipmentModes,
  fetchShipmentModeById,
  addShipmentMode,
  updateShipmentMode,
  deleteShipmentMode,
  clearShipmentModeMasterError,
  ShipmentModeMasterGridData,
} from "@/lib/shipmentModeMasterSlice";
...
export default function ShipmentModeMasterPage() {
  const dispatch = useAppDispatch();
  const { items, total, loading, error } = useAppSelector((s) => s.shipmentModeMaster);
  const { toast } = useToast();

  const [search, setSearch] = useState("");
  const [statusFilter, setStatusFilter] = useState<string>("ALL");
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editing, setEditing] = useState<ShipmentModeMasterGridData | null>(null);
  ...
  const uniqueStatuses = useMemo(() => {
    ...
    data.forEach((d: any) => {
      const s = d.statusEntry || d.STATUS_ENTRY;
  ...
  const filtered = useMemo(() => {
    return data.filter((d: any) => {
      const status = normalizeStatus(d.statusEntry || d.STATUS_ENTRY);
      const matchesStatus = statusFilter === "ALL" || status === statusFilter;
      if (!matchesStatus) return false;
      const searchable = [d.shipmentModeId, d.SHIPMENT_MODE_ID, d.shipmentModeName, d.SHIPMENT_MODE_NAME, d.remarks, d.REMARKS]
        .join(" ")
        .toLowerCase();
      return searchable.includes(search.toLowerCase());
    }).sort((a: any, b: any) => Number(b.id || b.shipmentModeId) - Number(a.id || a.shipmentModeId));
  ...
  useEffect(() => {
    if (error) {
      ...
      dispatch(clearShipmentModeMasterError());
    }
  ...
  const openEdit = async (item: any) => {
    ...
    const detail = item && item.id != null
      ? await dispatch(fetchShipmentModeById(Number(item.id))).unwrap().catch(() => null)
      : null;
    const src: any = detail ? detail : item;
    setForm({
      SHIPMENT_MODE_NAME: pickVal(src, ["shipmentModeName", "SHIPMENT_MODE_NAME"]),
      REMARKS: pickVal(src, ["remarks", "REMARKS"]),
      STATUS_ENTRY: pickVal(src, ["statusEntry", "STATUS_ENTRY"]) || "AC",
      USER: "Admin",
      MAC_ADDRESS: "WEB",
    });
    setDialogOpen(true);
  ...
  const handleSave = async () => {
    if (!form.SHIPMENT_MODE_NAME?.trim()) {
      toast({ title: "Shipment Mode Name is required", variant: "destructive", duration: DEFAULT_TOAST_DURATION });
      return;
    }

    if (!editing && normalizeStatus(form.STATUS_ENTRY) === "INACTIVE") {
      toast({ title: "New shipment mode cannot be set to Inactive", variant: "destructive", duration: DEFAULT_TOAST_DURATION });
      return;
    }

    try {
      const payload: Record<string, any> = {
        SHIPMENT_MODE_NAME: form.SHIPMENT_MODE_NAME?.trim(),
        REMARKS: form.REMARKS?.trim() || null,
        STATUS_ENTRY: form.STATUS_ENTRY || "AC",
        USER: "Admin",
        MAC_ADDRESS: "WEB",
      };
      ...
      if (editing) {
        ...
        payload.SHIPMENT_MODE_ID = Number(editing.id) || editing.shipmentModeId;
        const res = await dispatch(updateShipmentMode(payload)).unwrap();
        toast({ title: res?.message ?? "Shipment Mode updated successfully", duration: DEFAULT_TOAST_DURATION });
      } else {
        const res = await dispatch(addShipmentMode(payload)).unwrap();
        toast({ title: res?.message ?? "Shipment Mode created successfully", duration: DEFAULT_TOAST_DURATION });
      }
      setDialogOpen(false);
      dispatch(fetchShipmentModes({}));
    } catch (e: any) {
      toast({ title: typeof e === "string" ? e : (e?.message || "Error saving shipment mode"), variant: "destructive", duration: DEFAULT_TOAST_DURATION });
    }
  };
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      const res = await dispatch(deleteShipmentMode(deleteId)).unwrap();
      setDeleteId(null);
      toast({ title: res?.message ?? "Shipment Mode deleted successfully", duration: DEFAULT_TOAST_DURATION });
      dispatch(fetchShipmentModes({}));
    } catch (e: any) {
      toast({ title: typeof e === "string" ? e : (e?.message || "Error deleting shipment mode"), variant: "destructive", duration: DEFAULT_TOAST_DURATION });
    }
  };
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Shipment Mode Master</h1>
          <p className="text-sm text-muted-foreground">Manage shipment mode master data</p>
        </div>
        <Button onClick={openAdd} className="bg-primary text-primary-foreground hover:bg-primary/90">
          <Plus className="w-4 h-4 mr-2" /> Add Shipment Mode
        </Button>
      </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
          <div className="flex flex-1 flex-col sm:flex-row items-start sm:items-center gap-4 w-full sm:max-w-3xl">
            <div className="relative w-full sm:max-w-md">
              ...
              <Input placeholder="Search shipment modes..." value={search} onChange={(e) => { setSearch(e.target.value); setCurrentPage(1); }} className="pl-9 h-9 text-xs" />
            </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b bg-muted/50">
                  ...
                  <th className="text-left p-3 font-semibold text-muted-foreground uppercase text-xs">Shipment Mode Name</th>
              ...
              <tbody>
                {paginated.map((item: any, idx: number) => {
                  const id = item.id ?? item.shipmentModeId;
                  const name = item.shipmentModeName ?? item.SHIPMENT_MODE_NAME;
                  const remarks = item.remarks ?? item.REMARKS;
                  const status = item.statusEntry ?? item.STATUS_ENTRY;
                  return (
                ...
                {paginated.length === 0 && (
                  <tr><td colSpan={4} className="p-8 text-center text-muted-foreground">No shipment modes found</td></tr>
                )}
              </tbody>
            </table>
          )}
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editing ? "Edit Shipment Mode" : "Add Shipment Mode"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-6">
            <div>
              <h3 className="text-sm font-semibold text-muted-foreground border-b pb-2 mb-3">Shipment Mode Information</h3>
              <div className="grid grid-cols-2 gap-4">
                {renderField("SHIPMENT_MODE_NAME", "Shipment Mode Name", "text", undefined, true, "e.g., Sea Freight", 200)}
              </div>
            </div>
            <div>
              <h3 className="text-sm font-semibold text-muted-foreground border-b pb-2 mb-3">Details</h3>
              <div className="grid grid-cols-2 gap-4">
                {renderField("REMARKS", "Remarks", "textarea", undefined, false, "Additional notes...", 1000)}
              </div>
            </div>
            <div>
              <h3 className="text-sm font-semibold text-muted-foreground border-b pb-2 mb-3">Status</h3>
              <div className="grid grid-cols-2 gap-4">
                {renderField("STATUS_ENTRY", "Status", "select", [{ value: "AC", label: "Active" }, { value: "IA", label: "Inactive" }], false)}
              </div>
            </div>
      ...
      <AlertDialog open={!!deleteId} onOpenChange={(open) => !open && setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>This will permanently delete this shipment mode.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
