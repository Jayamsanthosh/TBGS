...
import { fetchPaymentTriggerEvents, addPaymentTriggerEvent, updatePaymentTriggerEvent, deletePaymentTriggerEvent, clearPaymentTriggerEventsError, type PaymentTriggerEventGridData } from "@/lib/paymentTriggerEventMasterSlice";
...
export default function PaymentTriggerEventMasterPage() {
  ...
  useEffect(() => {
    dispatch(fetchPaymentTriggerEvents(statusFilter));
  ...
  const filtered = useMemo(() => {
    ...
    return triggerEvents.filter((d: any) => {
      const searchable = [d.TRIGGER_EVENT_CODE, d.TRIGGER_EVENT_NAME, d.DESCRIPTION, d.REMARKS].join(" ").toLowerCase();
      return searchable.includes(search.toLowerCase());
    }).sort((a: any, b: any) => Number(b.TRIGGER_EVENT_ID || 0) - Number(a.TRIGGER_EVENT_ID || 0));
  }, [triggerEvents, search]);
  ...
  const openEdit = (item: any) => {
    setEditing(item);
    setForm({
      TRIGGER_EVENT_CODE: item.TRIGGER_EVENT_CODE || "",
  ...
  const handleSave = async () => {
    ...
    if (!form.TRIGGER_EVENT_CODE?.trim()) {
      toast({ title: "Trigger event code is required", variant: "destructive", duration: DEFAULT_TOAST_DURATION });
      return;
    }
    if (!form.TRIGGER_EVENT_NAME?.trim()) {
      toast({ title: "Trigger event name is required", variant: "destructive", duration: DEFAULT_TOAST_DURATION });
      return;
    }

    try {
      const payload: Record<string, any> = {
        TRIGGER_EVENT_CODE: form.TRIGGER_EVENT_CODE.trim(),
        TRIGGER_EVENT_NAME: form.TRIGGER_EVENT_NAME.trim(),
      ...
      if (editing) {
        payload.TRIGGER_EVENT_ID = Number(editing.id);
        const res = await dispatch(updatePaymentTriggerEvent(payload as PaymentTriggerEventGridData)).unwrap();
        toast({ title: res?.message ?? "Trigger event updated successfully", duration: DEFAULT_TOAST_DURATION });
      } else {
        const res = await dispatch(addPaymentTriggerEvent(payload as PaymentTriggerEventGridData)).unwrap();
        toast({ title: res?.message ?? "Trigger event created successfully", duration: DEFAULT_TOAST_DURATION });
      }
      setDialogOpen(false);
      dispatch(fetchPaymentTriggerEvents(statusFilter));
    } catch (e: any) {
      toast({ title: typeof e === 'string' ? e : (e?.payload || e?.message || "Error saving trigger event"), variant: "destructive", duration: DEFAULT_TOAST_DURATION });
    }
  };
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      const res = await dispatch(deletePaymentTriggerEvent(deleteId)).unwrap();
      setDeleteId(null);
      toast({ title: res?.message ?? "Trigger event deleted successfully", duration: DEFAULT_TOAST_DURATION });
      dispatch(fetchPaymentTriggerEvents(statusFilter));
    } catch (e: any) {
      toast({ title: typeof e === 'string' ? e : (e?.payload || e?.message || "Error deleting trigger event"), variant: "destructive", duration: DEFAULT_TOAST_DURATION });
    }
  };
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Payment Trigger Event Master</h1>
          <p className="text-sm text-muted-foreground">Manage payment trigger event master data</p>
        </div>
        <Button onClick={openAdd} className="bg-primary text-primary-foreground hover:bg-primary/90">
          <Plus className="w-4 h-4 mr-2" /> Add Trigger Event
        </Button>
      </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
          <div className="flex items-center gap-2 w-full sm:max-w-md">
            <div className="relative flex-1">
              ...
              <Input placeholder="Search trigger events..." value={search} onChange={(e) => { setSearch(e.target.value); setCurrentPage(1); }} className="pl-9 h-9 text-xs" />
            </div>
              ...
              <tbody>
                {paginated.map((item: any, idx: number) => (
                  <tr key={item.id || idx} className="border-b hover:bg-muted/30 transition-colors">
                    ...
                    <td className="p-3 font-medium">{item.TRIGGER_EVENT_ID}</td>
                    <td className="p-3">{item.TRIGGER_EVENT_CODE}</td>
                    <td className="p-3">{item.TRIGGER_EVENT_NAME}</td>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editing ? "Edit Trigger Event" : "Add Trigger Event"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-6">
            <div className="grid grid-cols-2 gap-4">
              <div className="col-span-2 sm:col-span-1">
                <Label className="text-xs">Trigger Event Code <span className="text-destructive">*</span></Label>
                <Input value={form.TRIGGER_EVENT_CODE || ""} onChange={(e) => setForm({ ...form, TRIGGER_EVENT_CODE: e.target.value })} placeholder="e.g., PAY001" className={`h-9 text-xs ${!form.TRIGGER_EVENT_CODE?.trim() ? "border-destructive ring-1 ring-destructive/30" : ""}`} />
              </div>
              <div className="col-span-2 sm:col-span-1">
                <Label className="text-xs">Trigger Event Name <span className="text-destructive">*</span></Label>
                <Input value={form.TRIGGER_EVENT_NAME || ""} onChange={(e) => setForm({ ...form, TRIGGER_EVENT_NAME: e.target.value })} placeholder="e.g., Payment Received" className={`h-9 text-xs ${!form.TRIGGER_EVENT_NAME?.trim() ? "border-destructive ring-1 ring-destructive/30" : ""}`} />
              </div>
              <div className="col-span-2">
                <Label className="text-xs">Description</Label>
                <Textarea value={form.DESCRIPTION || ""} onChange={(e) => setForm({ ...form, DESCRIPTION: e.target.value })} placeholder="Description of the trigger event..." className="text-xs" />
              </div>
              <div className="col-span-2">
      ...
      <AlertDialog open={!!deleteId} onOpenChange={(open) => !open && setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>This will permanently delete this trigger event record.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
