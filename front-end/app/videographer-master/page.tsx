...
import { fetchVideographers, addVideographer, updateVideographer, deleteVideographer, clearVideographerMasterError, type VideographerMasterGridData } from "@/lib/videographerMasterSlice";
...
export default function VideographerMasterPage() {
  const dispatch = useAppDispatch();
  const { videographers, loading, error } = useAppSelector((s) => s.videographerMaster);
  ...
  useEffect(() => {
    dispatch(fetchVideographers(statusFilter));
  }, [dispatch, statusFilter]);

  useEffect(() => {
    if (error) {
      toast({ variant: 'destructive', title: 'Error', description: error, duration: DEFAULT_TOAST_DURATION });
      dispatch(clearVideographerMasterError());
    }
  }, [error, dispatch, toast]);

  const filtered = useMemo(() => {
    if (!Array.isArray(videographers)) return [];
    return videographers.filter((d: any) => {
      const searchable = [d.VIDEOGRAPHER_NAME, d.ADDRESS, d.LOCATION_NAME, d.REMARKS].join(" ").toLowerCase();
      return searchable.includes(search.toLowerCase());
    }).sort((a: any, b: any) => Number(b.id || b.VIDEOGRAPHER_ID || 0) - Number(a.id || a.VIDEOGRAPHER_ID || 0));
  }, [videographers, search]);
  ...
  const openEdit = (item: any) => {
    setEditing(item);
    setForm({
      VIDEOGRAPHER_NAME: item.VIDEOGRAPHER_NAME || "",
  ...
  const handleSave = async () => {
    if (!form.VIDEOGRAPHER_NAME?.trim()) {
      toast({ title: "Videographer name is required", variant: "destructive", duration: DEFAULT_TOAST_DURATION });
      return;
    }
    ...
    try {
      const payload: Record<string, any> = {
        VIDEOGRAPHER_NAME: form.VIDEOGRAPHER_NAME.trim(),
      ...
      if (editing) {
        payload.VIDEOGRAPHER_ID = Number(editing.id);
        const res = await dispatch(updateVideographer(payload as VideographerMasterGridData)).unwrap();
        toast({ title: res?.message ?? "Videographer updated successfully", duration: DEFAULT_TOAST_DURATION });
      } else {
        const res = await dispatch(addVideographer(payload as VideographerMasterGridData)).unwrap();
        toast({ title: res?.message ?? "Videographer created successfully", duration: DEFAULT_TOAST_DURATION });
      }
      setDialogOpen(false);
      dispatch(fetchVideographers(statusFilter));
    } catch (e: any) {
      toast({ title: typeof e === 'string' ? e : (e?.payload || e?.message || "Error saving videographer"), variant: "destructive", duration: DEFAULT_TOAST_DURATION });
    }
  };
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      const res = await dispatch(deleteVideographer(deleteId)).unwrap();
      setDeleteId(null);
      toast({ title: res?.message ?? "Videographer deleted successfully", duration: DEFAULT_TOAST_DURATION });
      dispatch(fetchVideographers(statusFilter));
    } catch (e: any) {
      toast({ title: typeof e === 'string' ? e : (e?.payload || e?.message || "Error deleting videographer"), variant: "destructive", duration: DEFAULT_TOAST_DURATION });
    }
  };
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Videographer Master</h1>
          <p className="text-sm text-muted-foreground">Manage videographer master data</p>
        </div>
        <Button onClick={openAdd} className="bg-primary text-primary-foreground hover:bg-primary/90">
          <Plus className="w-4 h-4 mr-2" /> Add Videographer
        </Button>
      </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
          <div className="flex items-center gap-2 w-full sm:max-w-md">
            <div className="relative flex-1">
              ...
              <Input placeholder="Search videographers..." value={search} onChange={(e) => { setSearch(e.target.value); setCurrentPage(1); }} className="pl-9 h-9 text-xs" />
            </div>
              ...
              <tbody>
                {paginated.map((item: any, idx: number) => (
                  <tr key={item.id || idx} className="border-b hover:bg-muted/30 transition-colors">
                    ...
                    <td className="p-3 font-medium">{item.VIDEOGRAPHER_ID}</td>
                    <td className="p-3">{item.VIDEOGRAPHER_NAME}</td>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editing ? "Edit Videographer" : "Add Videographer"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-6">
            <div className="grid grid-cols-2 gap-4">
              <div className="col-span-2">
                <Label className="text-xs">Videographer Name <span className="text-destructive">*</span></Label>
                <Input value={form.VIDEOGRAPHER_NAME || ""} onChange={(e) => setForm({ ...form, VIDEOGRAPHER_NAME: e.target.value })} placeholder="e.g., John Doe" className={`h-9 text-xs${!form.VIDEOGRAPHER_NAME?.trim() ? " border-destructive ring-1 ring-destructive/30" : ""}`} />
              </div>
              <div className="col-span-2">
                <Label className="text-xs">Address</Label>
                <Input value={form.ADDRESS || ""} onChange={(e) => setForm({ ...form, ADDRESS: e.target.value })} placeholder="Enter address" className="h-9 text-xs" />
              </div>
              <div className="col-span-2">
                <Label className="text-xs">Location Name</Label>
                <Input value={form.LOCATION_NAME || ""} onChange={(e) => setForm({ ...form, LOCATION_NAME: e.target.value })} placeholder="e.g., Conference Room A" className="h-9 text-xs" />
              </div>
              <div className="col-span-2">
      ...
      <AlertDialog open={!!deleteId} onOpenChange={(open) => !open && setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>This will permanently delete this videographer record.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
