...
import { fetchItems, addItem, updateItem, deleteItem, clearError, type DesignationGroupGridData } from "@/lib/designationGroupMasterSlice";
...
export default function DesignationGroupMasterPage() {
  const dispatch = useAppDispatch();
  const { items, loading, error } = useAppSelector((s) => s.designationGroupMaster);
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((d: any) => {
      const searchable = [d.DESIGNATION_GROUP_NAME, d.REMARKS].join(" ").toLowerCase();
      return searchable.includes(search.toLowerCase());
    }).sort((a: any, b: any) => (b.DESIGNATION_GROUP_ID || 0) - (a.DESIGNATION_GROUP_ID || 0));
  }, [items, search]);
  ...
  const openEdit = (item: any) => {
    setEditing(item);
    setForm({
      DESIGNATION_GROUP_NAME: item.DESIGNATION_GROUP_NAME || "",
  ...
  const handleSave = async () => {
    if (!form.DESIGNATION_GROUP_NAME?.trim()) {
      toast({ title: "Designation group name is required", variant: "destructive", duration: DEFAULT_TOAST_DURATION });
      return;
    }

    if (!editing) {
      const s = normalizeStatus(form.STATUS_MASTER);
      if (s === "INACTIVE") {
        toast({ title: "New designation group cannot be set to Inactive", variant: "destructive", duration: DEFAULT_TOAST_DURATION });
        return;
      }
    }
    ...
    try {
      const payload: Record<string, any> = {
        DESIGNATION_GROUP_NAME: form.DESIGNATION_GROUP_NAME.trim(),
        REMARKS: form.REMARKS?.trim() || null,
        STATUS_MASTER: !editing ? "AC" : form.STATUS_MASTER,
        USER: "Admin",
        MAC_ADDRESS: "WEB",
      };

      if (editing) {
        payload.DESIGNATION_GROUP_ID = Number(editing.id);
        const res = await dispatch(updateItem(payload as DesignationGroupGridData)).unwrap();
        toast({ title: res?.message ?? "Designation group updated successfully", duration: DEFAULT_TOAST_DURATION });
      } else {
        const res = await dispatch(addItem(payload as DesignationGroupGridData)).unwrap();
        toast({ title: res?.message ?? "Designation group created successfully", duration: DEFAULT_TOAST_DURATION });
      }
      setDialogOpen(false);
      dispatch(fetchItems(statusFilter));
    } catch (e: any) {
      toast({ title: typeof e === 'string' ? e : (e?.payload || e?.message || "Error saving designation group"), variant: "destructive", duration: DEFAULT_TOAST_DURATION });
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setDeleteId(null);
      toast({ title: res?.message ?? "Designation group deleted successfully", duration: DEFAULT_TOAST_DURATION });
      dispatch(fetchItems(statusFilter));
    } catch (e: any) {
      toast({ title: typeof e === 'string' ? e : (e?.payload || e?.message || "Error deleting designation group"), variant: "destructive", duration: DEFAULT_TOAST_DURATION });
    }
  };
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Designation Group Master</h1>
          <p className="text-sm text-muted-foreground">Manage designation group master data</p>
        </div>
        <Button onClick={openAdd} className="bg-primary text-primary-foreground hover:bg-primary/90">
          <Plus className="w-4 h-4 mr-2" /> Add Designation Group
        </Button>
      </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
          <div className="flex items-center gap-2 w-full sm:max-w-md">
            <div className="relative flex-1">
              ...
              <Input placeholder="Search designation groups..." value={search} onChange={(e) => { setSearch(e.target.value); setCurrentPage(1); }} className="pl-9 h-9 text-xs" />
            </div>
            ...
            <div className="w-full space-y-4">
              {[...Array(5)].map((_, i) => (
                <div key={i} className="flex items-center space-x-4 py-4 border-b">
                  ...
                  {[...Array(3)].map((_, j) => <Skeleton key={j} className="h-4 flex-1" />)}
                </div>
              ))}
            </div>
          ) : (
              ...
              <tbody>
                {paginated.map((item: any, idx: number) => (
                  <tr key={item.id || idx} className="border-b hover:bg-muted/30 transition-colors">
                    ...
                    <td className="p-3 font-medium">{item.DESIGNATION_GROUP_ID}</td>
                    <td className="p-3">{item.DESIGNATION_GROUP_NAME}</td>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editing ? "Edit Designation Group" : "Add Designation Group"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-6">
            <div className="grid grid-cols-2 gap-4">
              <div className="col-span-2">
                <Label className="text-xs">Designation Group Name <span className="text-destructive">*</span></Label>
                <Input value={form.DESIGNATION_GROUP_NAME || ""} onChange={(e) => setForm({ ...form, DESIGNATION_GROUP_NAME: e.target.value })} placeholder="e.g., Senior Management" className={`h-9 text-xs ${!form.DESIGNATION_GROUP_NAME?.trim() ? "border-destructive ring-1 ring-destructive/30" : ""}`} />
              </div>
              <div className="col-span-2">
      ...
      <AlertDialog open={!!deleteId} onOpenChange={(open) => !open && setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>This will permanently delete this designation group record.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
