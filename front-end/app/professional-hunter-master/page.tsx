...
import { fetchProfessionalHunterMasters, addProfessionalHunterMaster, updateProfessionalHunterMaster, deleteProfessionalHunterMaster, fetchProfessionalHunterMasterById, clearProfessionalHunterMasterError } from "@/lib/professionalHunterMasterSlice";
...
export default function ProfessionalHunterMasterPage() {
  const dispatch = useAppDispatch();
  const { records, loading, error } = useAppSelector((s) => s.professionalHunterMaster);
  ...
  const filtered = useMemo(() => {
    ...
    return records.filter((d: any) => {
      const searchable = [d.PH_NAME, d.PH_DESCRIPTION].join(" ").toLowerCase();
      return searchable.includes(search.toLowerCase());
    }).sort((a: any, b: any) => Number(b.id || b.PH_ID) - Number(a.id || a.PH_ID));
  }, [records, search]);
  ...
  const openEdit = async (item: any) => {
    ...
    setEditing(item);
    try {
      const response = await dispatch(fetchProfessionalHunterMasterById(Number(item.PH_ID))).unwrap();
      if (response) {
        setForm({
          PH_NAME: response.PH_NAME || "",
          PH_DESCRIPTION: response.PH_DESCRIPTION || "",
  ...
  const handleSave = async () => {
    if (!form.PH_NAME?.trim()) {
      toast({ title: "Hunter name is required", variant: "destructive", duration: DEFAULT_TOAST_DURATION });
      return;
    }

    if (!editing && normalizeStatus(form.STATUS_MASTER) === "INACTIVE") {
      toast({ title: "New hunter cannot be set to Inactive", variant: "destructive", duration: DEFAULT_TOAST_DURATION });
      return;
    }

    try {
      const payload: Record<string, any> = {
        PH_NAME: form.PH_NAME.trim(),
        PH_DESCRIPTION: form.PH_DESCRIPTION?.trim() || null,
      ...
      if (editing) {
        ...
        payload.PH_ID = Number(editing.id);
        const res = await dispatch(updateProfessionalHunterMaster(payload)).unwrap();
        toast({ title: res?.message ?? "Hunter updated successfully", duration: DEFAULT_TOAST_DURATION });
      } else {
        const res = await dispatch(addProfessionalHunterMaster(payload)).unwrap();
        toast({ title: res?.message ?? "Hunter created successfully", duration: DEFAULT_TOAST_DURATION });
      }
      setDialogOpen(false);
      dispatch(fetchProfessionalHunterMasters());
    } catch (e: any) {
      toast({ title: typeof e === "string" ? e : (e?.message || "Error saving hunter"), variant: "destructive", duration: DEFAULT_TOAST_DURATION });
    }
  };
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      const res = await dispatch(deleteProfessionalHunterMaster(deleteId)).unwrap();
      setDeleteId(null);
      toast({ title: res?.message ?? "Hunter deleted successfully", duration: DEFAULT_TOAST_DURATION });
      dispatch(fetchProfessionalHunterMasters());
    } catch (e: any) {
      toast({ title: typeof e === "string" ? e : (e?.message || "Error deleting hunter"), variant: "destructive", duration: DEFAULT_TOAST_DURATION });
    }
  };
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Professional Hunter Master</h1>
          <p className="text-sm text-muted-foreground">Manage professional hunters</p>
        </div>
        <Button onClick={openAdd} className="bg-primary text-primary-foreground hover:bg-primary/90">
          <Plus className="w-4 h-4 mr-2" /> Add Hunter
        </Button>
      </div>
      ...
      <div className="bg-card rounded-xl border p-4 sm:p-6 shadow-sm overflow-hidden w-full">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
          <div className="relative w-full sm:max-w-md">
            ...
            <Input placeholder="Search hunters..." value={search} onChange={(e) => { setSearch(e.target.value); setCurrentPage(1); }} className="pl-9 h-9 text-xs" />
          </div>
              ...
              <tbody>
                {paginated.map((item: any, idx: number) => (
                  <tr key={item.id || idx} className="border-b hover:bg-muted/30 transition-colors">
                    ...
                    <td className="p-3 font-medium">{item.PH_ID}</td>
                    <td className="p-3">{item.PH_NAME}</td>
                    <td className="p-3">{item.PH_DESCRIPTION || "-"}</td>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editing ? "Edit Hunter" : "Add Hunter"}</DialogTitle>
          </DialogHeader>
          {loadingEdit ? (
          ...
          <div className="space-y-6">
            <div>
              <h3 className="text-sm font-semibold text-muted-foreground border-b pb-2 mb-3">Hunter Details</h3>
              <div className="grid grid-cols-2 gap-4">
                {renderField("PH_NAME", "Hunter Name", "text", undefined, true, "e.g., John Doe")}
                {renderField("PH_DESCRIPTION", "Description", "text", undefined, false, "e.g., Wildlife specialist")}
              </div>
            </div>
            <div>
      ...
      <AlertDialog open={!!deleteId} onOpenChange={(open) => !open && setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>This will permanently delete this hunter.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
