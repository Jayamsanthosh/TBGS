...
export default function GunMasterPage() {
  ...
  const [gunOwners, setGunOwners] = useState<DropdownItem[]>([]);
  const [gunSourceTypes, setGunSourceTypes] = useState<DropdownItem[]>([]);
  ...
  const fetchDropdowns = useCallback(async () => {
    const fetches = [
      ...
      fetch(`${API_URL}/business-partner-master`).then(r => r.json()).then(d => setGunOwners((d.data || []).filter((x: any) => x.STATUS_MASTER === 'AC').map((x: any) => ({ value: String(x.BP_ID ?? x.id), label: x.BP_NAME })))).catch(() => {}),
      fetch(`${API_URL}/gun-source-type-master`).then(r => r.json()).then(d => setGunSourceTypes((d.data || []).filter((x: any) => x.STATUS_MASTER === 'AC').map((x: any) => ({ value: String(x.GUN_SOURCE_TYPE_ID ?? x.id), label: x.GUN_SOURCE_TYPE_NAME })))).catch(() => {}),
    ];
  ...
  const filtered = useMemo(() => {
    ...
    return guns.filter((d: any) => {
      const searchable = [d.GUN_CODE, d.GUN_NAME, d.GUN_CATEGORY_NAME, d.TYPE_NAME, d.BRAND_Name, d.CALIBER_NAME, d.MODEL, d.SERIAL_NUMBER, d.LICENSE_NUMBER, d.CAMP_NAME, d.STORE_name, d.BP_NAME, d.GUN_SOURCE_TYPE_NAME].join(" ").toLowerCase();
    ...
    try {
      ...
      if (fetchGunMasterById.fulfilled.match(resultAction)) {
        ...
        setForm({
          ...
          REMARKS: raw.REMARKS || "",
          GUN_OWNER_BP_ID: raw.GUN_OWNER_BP_ID == null || raw.GUN_OWNER_BP_ID === 0 ? "" : String(raw.GUN_OWNER_BP_ID),
          GUN_SOURCE_TYPE_ID: raw.GUN_SOURCE_TYPE_ID == null || raw.GUN_SOURCE_TYPE_ID === 0 ? "" : String(raw.GUN_SOURCE_TYPE_ID),
  ...
  const handleSave = async () => {
    ...
    try {
      const payload: Record<string, any> = {
        ...
        GUN_OWNER_BP_ID: form.GUN_OWNER_BP_ID ? Number(form.GUN_OWNER_BP_ID) : null,
        GUN_SOURCE_TYPE_ID: form.GUN_SOURCE_TYPE_ID ? Number(form.GUN_SOURCE_TYPE_ID) : null,
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b bg-muted/50">
                  ...
                  <th className="text-left p-3 font-semibold text-muted-foreground uppercase text-xs">Owner</th>
                  <th className="text-left p-3 font-semibold text-muted-foreground uppercase text-xs">Source Type</th>
              ...
              <tbody>
                ...
                {paginated.length === 0 && (
                  <tr><td colSpan={15} className="p-8 text-center text-muted-foreground">No records found</td></tr>
                )}
              </tbody>
            </table>
          )}
          ...
          {loadingEdit ? (
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                ...
                {renderField("GUN_OWNER_BP_ID", "Gun Owner", "select", gunOwners, false, "Select Owner", undefined, false)}
                {renderField("GUN_SOURCE_TYPE_ID", "Source Type", "select", gunSourceTypes, false, "Select Source Type", undefined, false)}
