...
import { fetchBookingSources, addBookingSource, updateBookingSource, deleteBookingSource, clearBookingSourceError, type BookingSourceGridData } from "@/lib/bookingSourceMasterSlice";
...
export default function BookingSourceMasterPage() {
  const dispatch = useAppDispatch();
  const { bookingSources, loading, error } = useAppSelector((s) => s.bookingSource);
  ...
  const filtered = useMemo(() => {
    if (!Array.isArray(bookingSources)) return [];
    return bookingSources.filter((d: any) => {
      const searchable = [d.BOOKING_SOURCE_NAME, d.BOOKING_SOURCE_DESCRIPTION, d.REMARKS, d.STATUS_MASTER].join(" ").toLowerCase();
      ...
      return true;
    }).sort((a: any, b: any) => Number(b.BOOKING_SOURCE_ID) - Number(a.BOOKING_SOURCE_ID));
  }, [bookingSources, search, statusFilter]);
  ...
  const openEdit = (item: any) => {
    setEditing(item);
    setForm({
      BOOKING_SOURCE_ID: item.BOOKING_SOURCE_ID || 0,
      BOOKING_SOURCE_NAME: item.BOOKING_SOURCE_NAME || "",
      BOOKING_SOURCE_DESCRIPTION: item.BOOKING_SOURCE_DESCRIPTION || "",
  ...
  const handleSave = async () => {
    if (!form.BOOKING_SOURCE_NAME?.trim()) {
      toast({ title: "Booking source name is required", variant: "destructive", duration: DEFAULT_TOAST_DURATION });
      return;
    }
    if (form.BOOKING_SOURCE_NAME.trim().length > 50) {
      toast({ title: "Booking source name must be 50 characters or less", variant: "destructive", duration: DEFAULT_TOAST_DURATION });
      return;
    }
    if (form.BOOKING_SOURCE_DESCRIPTION && form.BOOKING_SOURCE_DESCRIPTION.trim().length > 50) {
    ...
    if (!editing && normalizeStatus(form.STATUS_MASTER) === "INACTIVE") {
      toast({ title: "New booking source cannot be set to Inactive", variant: "destructive", duration: DEFAULT_TOAST_DURATION });
      return;
    }

    try {
      const payload: Record<string, any> = {
        BOOKING_SOURCE_NAME: form.BOOKING_SOURCE_NAME.trim(),
        BOOKING_SOURCE_DESCRIPTION: form.BOOKING_SOURCE_DESCRIPTION?.trim() || null,
      ...
      if (editing) {
        payload.BOOKING_SOURCE_ID = Number(editing.id);
        const res = await dispatch(updateBookingSource(payload as BookingSourceGridData)).unwrap();
        toast({ title: res?.message ?? "Booking source updated successfully", duration: DEFAULT_TOAST_DURATION });
      } else {
        const res = await dispatch(addBookingSource(payload as BookingSourceGridData)).unwrap();
        toast({ title: res?.message ?? "Booking source created successfully", duration: DEFAULT_TOAST_DURATION });
      }
      setDialogOpen(false);
      dispatch(fetchBookingSources(statusFilter));
    } catch (e: any) {
      toast({ title: typeof e === "string" ? e : (e?.message || "Error saving booking source"), variant: "destructive", duration: DEFAULT_TOAST_DURATION });
    }
  };
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      const res = await dispatch(deleteBookingSource(deleteId)).unwrap();
      setDeleteId(null);
      toast({ title: res?.message ?? "Booking source deleted successfully", duration: DEFAULT_TOAST_DURATION });
      dispatch(fetchBookingSources(statusFilter));
    } catch (e: any) {
      toast({ title: typeof e === "string" ? e : (e?.message || "Error deleting booking source"), variant: "destructive", duration: DEFAULT_TOAST_DURATION });
    }
  };
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Booking Source Master</h1>
          <p className="text-sm text-muted-foreground">Manage booking source master data</p>
        </div>
        <Button onClick={openAdd} className="bg-primary text-primary-foreground hover:bg-primary/90">
          <Plus className="w-4 h-4 mr-2" /> Add Booking Source
        </Button>
      </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
          <div className="flex items-center gap-2 w-full sm:max-w-md">
            <div className="relative flex-1">
              ...
              <Input placeholder="Search booking sources..." value={search} onChange={(e) => { setSearch(e.target.value); setCurrentPage(1); }} className="pl-9 h-9 text-xs" />
            </div>
              ...
              <tbody>
                {paginated.map((item: any, idx: number) => (
                  <tr key={item.id || idx} className="border-b hover:bg-muted/30 transition-colors">
                    ...
                    <td className="p-3 font-medium">{item.BOOKING_SOURCE_ID}</td>
                    <td className="p-3">{item.BOOKING_SOURCE_NAME}</td>
                    <td className="p-3">{item.BOOKING_SOURCE_DESCRIPTION || "-"}</td>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editing ? "Edit Booking Source" : "Add Booking Source"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-6">
            <div className="grid grid-cols-2 gap-4">
              {renderField("BOOKING_SOURCE_NAME", "Booking Source Name", "text", undefined, true, "e.g., Safari Agent", 50)}
              {renderField("BOOKING_SOURCE_DESCRIPTION", "Description", "text", undefined, false, "e.g., Booking via agent", 50)}
      ...
      <AlertDialog open={!!deleteId} onOpenChange={(open) => !open && setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>This will permanently delete this booking source.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
