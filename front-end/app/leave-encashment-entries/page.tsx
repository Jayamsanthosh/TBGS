...
export default function LeaveEncashmentEntriesPage() {
  ...
  const handleSubmit = useCallback(async (item: any) => {
    ...
    try {
      const res = await dispatch(submitLeaveEncashmentEntries({ ...item, LEAVE_ENCASHMENT_REQUEST_REF_NO: ref } as LeaveEncashmentEntriesGridData)).unwrap();
      toast({ title: res?.message ?? "Leave encashment entry submitted successfully!" });
      dispatch(fetchLeaveEncashmentEntries({ status: currentStatus, fromDate, toDate }));
    } catch (e: any) {
      const msg = typeof e === "string" ? e : (e?.message || "Failed to submit leave encashment entry");
  ...
  return (
    <div className="space-y-4">
      <MasterCrudPage
        title="Leave Encashment Response"
        description="Manage leave encashment responses and salary components"
        idPrefix="LEE"
        ...
        rowActions={(item) => {
          ...
          return (
            <button
              ...
              disabled={submitting}
              title="Submit leave encashment entry"
