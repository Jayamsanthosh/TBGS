...
import { fetchBookingStatuses, addBookingStatus, updateBookingStatus, deleteBookingStatus, clearBookingStatusError, type BookingStatusGridData } from "@/lib/bookingStatusMasterSlice";
...
export default function BookingStatusMasterPage() {
  const dispatch = useAppDispatch();
  const { bookingStatuses, loading, error } = useAppSelector((s) => s.bookingStatus);
  ...
  const filtered = useMemo(() => {
    if (!Array.isArray(bookingStatuses)) return [];
    return bookingStatuses.filter((d: any) => {
      const searchable = [d.BOOKING_STATUS_NAME, d.BOOKING_STATUS_DESCRIPTION, d.REMARKS, d.STATUS_MASTER].join(" ").toLowerCase();
      ...
      return true;
    }).sort((a: any, b: any) => Number(b.BOOKING_STATUS_ID) - Number(a.BOOKING_STATUS_ID));
  }, [bookingStatuses, search, statusFilter]);
  ...
  const openEdit = (item: any) => {
    setEditing(item);
    setForm({
      BOOKING_STATUS_ID: item.BOOKING_STATUS_ID || 0,
      BOOKING_STATUS_NAME: item.BOOKING_STATUS_NAME || "",
      BOOKING_STATUS_DESCRIPTION: item.BOOKING_STATUS_DESCRIPTION || "",
  ...
  const handleSave = async () => {
    if (!form.BOOKING_STATUS_NAME?.trim()) {
      toast({ title: "Booking status name is required", variant: "destructive", duration: DEFAULT_TOAST_DURATION });
      return;
    }
    if (form.BOOKING_STATUS_NAME.trim().length > 50) {
      toast({ title: "Booking status name must be 50 characters or less", variant: "destructive", duration: DEFAULT_TOAST_DURATION });
      return;
    }
    if (form.BOOKING_STATUS_DESCRIPTION && form.BOOKING_STATUS_DESCRIPTION.trim().length > 50) {
    ...
    if (!editing && normalizeStatus(form.STATUS_MASTER) === "INACTIVE") {
      toast({ title: "New booking status cannot be set to Inactive", variant: "destructive", duration: DEFAULT_TOAST_DURATION });
      return;
    }

    try {
      const payload: Record<string, any> = {
        BOOKING_STATUS_NAME: form.BOOKING_STATUS_NAME.trim(),
        BOOKING_STATUS_DESCRIPTION: form.BOOKING_STATUS_DESCRIPTION?.trim() || null,
      ...
      if (editing) {
        payload.BOOKING_STATUS_ID = Number(editing.id);
        const res = await dispatch(updateBookingStatus(payload as BookingStatusGridData)).unwrap();
        toast({ title: res?.message ?? "Booking status updated successfully", duration: DEFAULT_TOAST_DURATION });
      } else {
        const res = await dispatch(addBookingStatus(payload as BookingStatusGridData)).unwrap();
        toast({ title: res?.message ?? "Booking status created successfully", duration: DEFAULT_TOAST_DURATION });
      }
      setDialogOpen(false);
      dispatch(fetchBookingStatuses(statusFilter));
    } catch (e: any) {
      toast({ title: typeof e === "string" ? e : (e?.message || "Error saving booking status"), variant: "destructive", duration: DEFAULT_TOAST_DURATION });
    }
  };
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      const res = await dispatch(deleteBookingStatus(deleteId)).unwrap();
      setDeleteId(null);
      toast({ title: res?.message ?? "Booking status deleted successfully", duration: DEFAULT_TOAST_DURATION });
      dispatch(fetchBookingStatuses(statusFilter));
    } catch (e: any) {
      toast({ title: typeof e === "string" ? e : (e?.message || "Error deleting booking status"), variant: "destructive", duration: DEFAULT_TOAST_DURATION });
    }
  };
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Booking Status Master</h1>
          <p className="text-sm text-muted-foreground">Manage booking status master data</p>
        </div>
        <Button onClick={openAdd} className="bg-primary text-primary-foreground hover:bg-primary/90">
          <Plus className="w-4 h-4 mr-2" /> Add Booking Status
        </Button>
      </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
          <div className="flex items-center gap-2 w-full sm:max-w-md">
            <div className="relative flex-1">
              ...
              <Input placeholder="Search booking statuses..." value={search} onChange={(e) => { setSearch(e.target.value); setCurrentPage(1); }} className="pl-9 h-9 text-xs" />
            </div>
              ...
              <tbody>
                {paginated.map((item: any, idx: number) => (
                  <tr key={item.id || idx} className="border-b hover:bg-muted/30 transition-colors">
                    ...
                    <td className="p-3 font-medium">{item.BOOKING_STATUS_ID}</td>
                    <td className="p-3">{item.BOOKING_STATUS_NAME}</td>
                    <td className="p-3">{item.BOOKING_STATUS_DESCRIPTION || "-"}</td>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editing ? "Edit Booking Status" : "Add Booking Status"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-6">
            <div className="grid grid-cols-2 gap-4">
              {renderField("BOOKING_STATUS_NAME", "Booking Status Name", "text", undefined, true, "e.g., Confirmed", 50)}
              {renderField("BOOKING_STATUS_DESCRIPTION", "Description", "text", undefined, false, "e.g., Booking confirmed", 50)}
      ...
      <AlertDialog open={!!deleteId} onOpenChange={(open) => !open && setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>This will permanently delete this booking status.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
