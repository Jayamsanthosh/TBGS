...
export default function DeductionEntriesPage() {
  ...
  const handleSubmit = useCallback(async (item: any) => {
    const ref = item?.DED_REF_ID;
    if (!ref) { toast({ variant: "destructive", title: "Error", description: "Missing reference number" }); return; }
    setSubmittingId(item.id ?? item.DED_REF_ID);
    try {
      const res = await dispatch(submitDeductionEntries({ ...item, DED_REF_ID: ref } as DeductionEntriesGridData)).unwrap();
      toast({ title: res?.message ?? "Deduction entry submitted successfully!" });
      dispatch(fetchDeductionEntries({ status: currentStatus, fromDate, toDate }));
    } catch (e: any) {
      const msg = typeof e === "string" ? e : (e?.message || "Failed to submit deduction entry");
  ...
  return (
    <MasterCrudPage
      ...
      rowActions={(item) => {
        ...
        const submitting = submittingId != null && String(submittingId) === String(item.id ?? item.DED_REF_ID);
        return (
          <button
            onClick={() => handleSubmit(item)}
            disabled={submitting}
            title="Submit deduction entry"
