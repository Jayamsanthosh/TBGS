...
import {
  ...
  deleteBonusEntries,
  submitBonusEntries,
  clearBonusEntriesError,
  BonusEntriesGridData,
} from "@/lib/bonusEntriesSlice";
import { useApiQuery } from "@/lib/reduxQuery";
import { API_URL } from "@/lib/config";
import { useToast } from "@/hooks/use-toast";
import { Badge } from "@/components/ui/badge";
import { EmployeeCombobox } from "@/components/EmployeeCombobox";
import { MONTHS, YEARS, BONUS_TYPE } from "@/lib/utils";
import {
...
const paidBadge = (val: any) => {
  const sv = String(val || "").toLowerCase();
  const isPaid = sv === "paid";
  const isUnpaid = sv === "unpaid";
...
export default function BonusEntriesPage() {
  const dispatch = useAppDispatch();
  const { items, loading, error } = useAppSelector((s) => s.bonusEntries);
  ...
  const handleSubmit = useCallback(async (item: any) => {
    const ref = item?.BONUS_REQUEST_REF_NO;
    if (!ref) { toast({ variant: "destructive", title: "Error", description: "Missing reference number" }); return; }
    setSubmittingId(item.id ?? item.SNO);
    try {
      const res = await dispatch(submitBonusEntries({ ...item, BONUS_REQUEST_REF_NO: ref } as BonusEntriesGridData)).unwrap();
      toast({ title: res?.message ?? "Bonus entry submitted successfully!" });
      dispatch(fetchBonusEntries({ status: currentStatus, fromDate, toDate }));
    } catch (e: any) {
      const msg = typeof e === "string" ? e : (e?.message || "Failed to submit bonus entry");
  ...
  const { data: userStoreMappings } = useApiQuery("bon-entry-user-map", async () => {
  ...
  const { data: employees } = useApiQuery("bon-entry-employees", async () => {
  ...
  const { data: companies } = useApiQuery("bon-entry-companies", async () => {
  ...
  const { data: departments } = useApiQuery("bon-entry-departments", async () => {
  ...
  const { data: designations } = useApiQuery("bon-entry-designations", async () => {
  ...
  const { data: departmentGroups } = useApiQuery("bon-entry-dept-groups", async () => {
  ...
  const { data: designationGroups } = useApiQuery("bon-entry-desig-groups", async () => {
  ...
  const { data: camps } = useApiQuery("bon-entry-camps", async () => {
  ...
  const { data: stores } = useApiQuery("bon-entry-stores", async () => {
  ...
  const { data: employmentTypes } = useApiQuery("bon-entry-employment-types", async () => {
  ...
  const { data: currencies } = useApiQuery("bon-entry-currencies", async () => {
  ...
  const { data: paymentModes } = useApiQuery("bon-entry-payment-modes", async () => {
  ...
  const { data: banks } = useApiQuery("bon-entry-banks", async () => {
  ...
  const { data: deptDesigMappings } = useApiQuery("bon-entry-dept-desig-map", async () => {
  ...
  const { data: campStoreMappings } = useApiQuery("bon-entry-camp-store-map", async () => {
  ...
  const baseFields: MasterField[] = [
    ...
    {
      key: "BONUS_REQUEST_REF_NO",
      label: "Bonus Request Ref No",
      type: "select",
      required: true,
      dependsOn: "EMP_ID",
      options: [],
    ...
    { key: "CURRENCY_ID", label: "Currency", type: "select", options: selectOptions("CURRENCY_ID", currencies, "CURRENCY_NAME"), disabled: true },
    {
      ...
      type: "select",
      required: true,
      options: BONUS_TYPE.map((s) => ({ value: s, label: s })),
    ...
    { key: "PAYMENT_REF_NO", label: "Payment Ref No", type: "text", placeholder: "Enter payment reference" },
    {
      key: "PAYMENT_MODE_ID",
      label: "Payment Mode",
      type: "select",
  ...
  const fields: MasterField[] = useMemo(() => {
    return baseFields.map((f) => {
      switch (f.key) {
        case "BONUS_REQUEST_REF_NO": return {
          ...f,
          options: (form: Record<string, any>) =>
            (Array.isArray(bonusRequests) ? bonusRequests : [])
              .filter((r: any) => String(r.EMP_ID) === String(form.EMP_ID) && r.BONUS_REQUEST_REF_NO)
              .map((r: any) => ({ value: String(r.BONUS_REQUEST_REF_NO), label: String(r.BONUS_REQUEST_REF_NO) })),
        };
  ...
  }, [companies, departments, designations, departmentGroups, designationGroups, camps, stores, employmentTypes, paymentModes, banks, currencies, cascadeMaps, bonusRequests]);

  const handleFieldChange = useCallback((key: string, value: any, setForm: any, form: Record<string, any>): boolean => {
    if (key === "BONUS_REQUEST_REF_NO") {
      const req = (Array.isArray(bonusRequests) ? bonusRequests : []).find(
        (r: any) => String(r.BONUS_REQUEST_REF_NO) === String(value)
      );
      if (req) {
        ...
        setForm({
          ...
          REQUEST_AMOUNT: toNum(req.REQUEST_AMOUNT),
          APPROVED_AMOUNT: toNum(req.APPROVED_AMOUNT ?? req.REQUEST_AMOUNT),
        ...
        setForm({ ...form, BONUS_REQUEST_REF_NO: value });
      }
      return true;
    }
    ...
    if (key === "EMP_ID") {
      ...
      if (empGrid && empGrid.SNO) {
        ...
        setForm({ ...form, EMP_ID: value, BONUS_REQUEST_REF_NO: "" });
      }
      return true;
    }

    return false;
  }, [filteredEmployees, bonusRequests]);

  const columns = useMemo(() => [
    { key: "SNO", label: "ID" },
    { key: "BONUS_REQUEST_REF_NO", label: "Ref No" },
  ...
  const enrichedData = useMemo(() => {
    ...
    return items.map((u: any) => {
      ...
      return {
        ...
        PAYMENT_MODE_NAME: toId("PAYMENT_MODE_ID")(paymentModes, u.PAYMENT_MODE_ID)?.PAYMENT_MODE_NAME || u.PAYMENT_MODE_NAME || "",
        BANK_NAME: toId("BANK_ID")(banks, u.BANK_ID)?.BANK_NAME || u.BANK_NAME || "",
      };
    });
  }, [items, employees, companies, departments, designations, paymentModes, banks]);
  ...
  useEffect(() => {
    if (error) {
      ...
      dispatch(clearBonusEntriesError());
    }
  ...
  const storeOverrides = useMemo(() => ({
    data: enrichedData,
    isLoading: loading,
    add: async (item: BonusEntriesGridData) => {
      const NUM_KEYS = [
        "YEAR_ENTERED",
        "EMP_ID",
        "COMPANY_ID",
      ...
      const res = await dispatch(addBonusEntries(payload as BonusEntriesGridData)).unwrap();
      dispatch(fetchBonusEntries({ status: currentStatus, fromDate, toDate }));
      return res;
    },
    update: async (item: BonusEntriesGridData) => {
      const res = await dispatch(updateBonusEntries(item)).unwrap();
      dispatch(fetchBonusEntries({ status: currentStatus, fromDate, toDate }));
      return res;
    },
    remove: async (id: string) => {
      const row = (Array.isArray(enrichedData) ? enrichedData.find((r: any) => String(r.SNO) === String(id)) : undefined);
      const ref = row?.BONUS_REQUEST_REF_NO || id;
      const res = await dispatch(deleteBonusEntries(ref)).unwrap();
      dispatch(fetchBonusEntries({ status: currentStatus, fromDate, toDate }));
      return res;
    },
    bulkRemove: async (ids: string[]) => {
      ...
      for (const id of ids) {
        const row = (Array.isArray(enrichedData) ? enrichedData.find((r: any) => String(r.SNO) === String(id)) : undefined);
        const ref = row?.BONUS_REQUEST_REF_NO || id;
        res = await dispatch(deleteBonusEntries(ref)).unwrap();
      }
      dispatch(fetchBonusEntries({ status: currentStatus, fromDate, toDate }));
      return res;
    },
  ...
  return (
    <div className="space-y-4">
      ...
      description="Manage bonus responses"
      idPrefix="BNE"
      domain="bonus-entries"
      fields={fields}
      columns={columns}
      initialData={[]}
      ...
      onBeforeEdit={async (item) => {
        ...
        const res = await fetch(`${API_URL}/bonus-entries/${encodeURIComponent(ref)}`);
