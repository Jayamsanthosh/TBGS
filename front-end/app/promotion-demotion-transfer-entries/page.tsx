...
export default function PromotionDemotionTransferEntriesPage() {
  ...
  const handleSubmit = useCallback(async (item: any) => {
    ...
    try {
      const res = await dispatch(submitPromotionDemotionTransferEntry({ ...item, TRANSFER_REQUEST_REF_NO: ref } as PromotionDemotionTransferEntriesGridData)).unwrap();
      toast({ title: res?.message ?? "Promotion / demotion / transfer entry submitted successfully!" });
      dispatch(fetchPromotionDemotionTransferEntries({ status: "ALL", fromDate, toDate }));
    } catch (e: any) {
      const msg = typeof e === "string" ? e : (e?.message || "Failed to submit promotion / demotion / transfer entry");
      toast({ variant: "destructive", title: "Error", description: msg });
    } finally {
      setSubmittingId(null);
    }
  }, [dispatch, fromDate, toDate, toast]);
  ...
  }), [enrichedData, loading, dispatch, fromDate, toDate, handleFieldChange]);

  return (
    <div className="space-y-4">
      <MasterCrudPage
        title="Promotion / Demotion / Transfer Response"
        description="Manage promotion, demotion and transfer responses"
        idPrefix="PDTE"
        ...
        rowActions={(item) => {
          ...
          return (
            <button
              ...
              disabled={submitting}
              title="Submit promotion / demotion / transfer entry"
