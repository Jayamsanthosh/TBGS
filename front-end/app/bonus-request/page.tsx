...
import {
  ...
  updateBonusRequest,
  deleteBonusRequest,
  clearBonusRequestError,
  BonusRequestGridData,
} from "@/lib/bonusRequestSlice";
...
const paidBadge = (val: any) => {
  ...
  const isUnpaid = sv === "unpaid" || sv === "un paid" || sv === "unpaid";
...
export default function BonusRequestPage() {
  ...
  const { items, loading, error } = useAppSelector((s) => s.bonusRequest);
  ...
  const { data: userStoreMappings } = useApiQuery("bon-req-user-map", async () => {
  ...
  const { data: employees } = useApiQuery("bon-req-employees", async () => {
  ...
  const { data: companies } = useApiQuery("bon-req-companies", async () => {
  ...
  const { data: departments } = useApiQuery("bon-req-departments", async () => {
  ...
  const { data: designations } = useApiQuery("bon-req-designations", async () => {
  ...
  const { data: departmentGroups } = useApiQuery("bon-req-dept-groups", async () => {
  ...
  const { data: designationGroups } = useApiQuery("bon-req-desig-groups", async () => {
  ...
  const { data: camps } = useApiQuery("bon-req-camps", async () => {
  ...
  const { data: stores } = useApiQuery("bon-req-stores", async () => {
  ...
  const { data: employmentTypes } = useApiQuery("bon-req-employment-types", async () => {
  ...
  const { data: currencies } = useApiQuery("bon-req-currencies", async () => {
  ...
  const { data: paymentModes } = useApiQuery("bon-req-payment-modes", async () => {
  ...
  const { data: banks } = useApiQuery("bon-req-banks", async () => {
  ...
  const { data: deptDesigMappings } = useApiQuery("bon-req-dept-desig-map", async () => {
  ...
  const { data: campStoreMappings } = useApiQuery("bon-req-camp-store-map", async () => {
  ...
  const baseFields: MasterField[] = [
    ...
    { key: "REQUEST_AMOUNT", label: "Request Amount", type: "number", required: true, placeholder: "0.00", disabledOnEdit: true, validate: (value: any) => {
    ...
    { key: "PAYMENT_REF_NO", label: "Payment Ref No", type: "text", placeholder: "Enter payment reference" },
    {
      ...
      // Auto-filled from employee table, but approver can change
    },
    {
      key: "BANK_ID",
      label: "Bank",
      ...
      // Auto-filled from employee table, but user can change
    },
  ...
  const handleFieldChange = useCallback((key: string, value: any, setForm: any, form: Record<string, any>): boolean => {
          ...
          .then(json => {
            if (json.success && json.data) {
              ...
              setForm((prev: any) => ({
                ...
                // Auto-fill payment details from employee (user can override after)
    ...
    // Auto-fill APPROVED_AMOUNT = REQUEST_AMOUNT by default (per table: "By default request amount")
    if (key === "REQUEST_AMOUNT") {
      setForm((prev: any) => ({
        ...prev,
        REQUEST_AMOUNT: value,
        // Only auto-fill if APPROVED_AMOUNT is not yet set or matches the old REQUEST_AMOUNT
        APPROVED_AMOUNT:
          !prev.APPROVED_AMOUNT || prev.APPROVED_AMOUNT === prev.REQUEST_AMOUNT
            ? value
            : prev.APPROVED_AMOUNT,
      }));
      return true;
    }
  ...
  useEffect(() => {
    dispatch(fetchBonusRequests(currentStatus));
  }, [dispatch, currentStatus]);

  useEffect(() => {
    if (error) {
      toast({ variant: "destructive", title: "Error", description: error });
      dispatch(clearBonusRequestError());
    }
  ...
  const storeOverrides = useMemo(() => ({
    data: enrichedData,
    isLoading: loading,
    add: async (item: BonusRequestGridData) => {
      const NUM_KEYS = [
        "YEAR_ENTERED",
        "EMP_ID",
        "COMPANY_ID",
      ...
      const payload: Record<string, any> = {
        ...
        BONUS_REQUEST_REF_NO: item.BONUS_REQUEST_REF_NO || `BRF/${new Date().toISOString().slice(0, 10).replace(/-/g, "")}/${Date.now()}`,
        // APPROVED_AMOUNT defaults to REQUEST_AMOUNT if not explicitly set
        APPROVED_AMOUNT: item.APPROVED_AMOUNT || item.REQUEST_AMOUNT,
      ...
      const res = await dispatch(addBonusRequest(payload as BonusRequestGridData)).unwrap();
      dispatch(fetchBonusRequests(currentStatus));
      return res;
    },
    update: async (item: BonusRequestGridData) => {
      const res = await dispatch(updateBonusRequest(item)).unwrap();
      dispatch(fetchBonusRequests(currentStatus));
      return res;
    },
    remove: async (id: string) => {
      ...
      const res = await dispatch(deleteBonusRequest(ref)).unwrap();
      dispatch(fetchBonusRequests(currentStatus));
      return res;
    },
    bulkRemove: async (ids: string[]) => {
      ...
      for (const id of ids) {
        const row = (Array.isArray(enrichedData) ? enrichedData.find((r: any) => String(r.SNO) === String(id)) : undefined);
        const ref = row?.BONUS_REQUEST_REF_NO || id;
        res = await dispatch(deleteBonusRequest(ref)).unwrap();
      }
      dispatch(fetchBonusRequests(currentStatus));
      return res;
    },
  ...
  return (
    <div className="space-y-4">
      <MasterCrudPage
      title="Bonus Request"
      description="Manage bonus requests"
      idPrefix="BRF"
      domain="bonus-request"
      fields={fields}
      columns={columns}
      initialData={[]}
      ...
      onBeforeEdit={async (item) => {
        ...
        const res = await fetch(`${API_URL}/bonus-request/${encodeURIComponent(ref)}`);
