...
import {
  ...
  clearLeaveEncashmentError,
  LeaveEncashmentGridData,
} from "@/lib/leaveEncashmentSlice";
...
export default function LeaveEncashmentRequestPage() {
  const dispatch = useAppDispatch();
  const { items, loading, error } = useAppSelector((s) => s.leaveEncashment);
  ...
  const { data: userStoreMappings } = useApiQuery("ler-user-map", async () => {
  ...
  const { data: employees } = useApiQuery("ler-employees", async () => {
  ...
  const { data: companies } = useApiQuery("ler-companies", async () => {
  ...
  const { data: departments } = useApiQuery("ler-departments", async () => {
  ...
  const { data: designations } = useApiQuery("ler-designations", async () => {
  ...
  const { data: departmentGroups } = useApiQuery("ler-dept-groups", async () => {
  ...
  const { data: designationGroups } = useApiQuery("ler-desig-groups", async () => {
  ...
  const { data: camps } = useApiQuery("ler-camps", async () => {
  ...
  const { data: stores } = useApiQuery("ler-stores", async () => {
  ...
  const { data: employmentTypes } = useApiQuery("ler-employment-types", async () => {
  ...
  const { data: currencies } = useApiQuery("ler-currencies", async () => {
  ...
  const { data: deptDesigMappings } = useApiQuery("ler-dept-desig-map", async () => {
  ...
  const { data: campStoreMappings } = useApiQuery("ler-camp-store-map", async () => {
  ...
  const baseFields: MasterField[] = [
    ...
    { key: "BALANCE_LEAVE_DAYS", label: "Balance Leave Days", type: "number", placeholder: "0.00", disabled: true, validate: (value: any) => {
      if (value != null && value !== "" && !validateNonNegativeNumber(value)) return "Balance days must be 0 or greater";
      return undefined;
    } },
    { key: "LEAVE_ENCASHMENT_DAYS", label: "Encashment Days", type: "number", required: true, placeholder: "0.00", validate: (value: any) => {
      if (value == null || value === "") return "Encashment days is required";
      if (!validatePositiveNumber(value)) return "Encashment days must be greater than 0";
      return undefined;
    } },
  ...
  useEffect(() => {
    dispatch(fetchLeaveEncashments(currentStatus));
  }, [dispatch, currentStatus]);

  useEffect(() => {
    if (error) {
      toast({ variant: "destructive", title: "Error", description: error });
      dispatch(clearLeaveEncashmentError());
    }
  ...
  const storeOverrides = useMemo(() => ({
    data: enrichedData,
    isLoading: loading,
    add: async (item: LeaveEncashmentGridData) => {
      const NUM_KEYS = [
        "YEAR_ENTERED",
        "EMP_ID",
        "COMPANY_ID",
      ...
      const res = await dispatch(addLeaveEncashment(payload as LeaveEncashmentGridData)).unwrap();
      dispatch(fetchLeaveEncashments(currentStatus));
      return res;
    },
    update: async (item: LeaveEncashmentGridData) => {
      const res = await dispatch(updateLeaveEncashment(item)).unwrap();
      dispatch(fetchLeaveEncashments(currentStatus));
      return res;
    },
    remove: async (id: string) => {
      ...
      const res = await dispatch(deleteLeaveEncashment(ref)).unwrap();
      dispatch(fetchLeaveEncashments(currentStatus));
      return res;
    },
    bulkRemove: async (ids: string[]) => {
      ...
      for (const id of ids) {
        const row = (Array.isArray(enrichedData) ? enrichedData.find((r: any) => String(r.SNO) === String(id)) : undefined);
        const ref = row?.LEAVE_ENCASHMENT_REQUEST_REF_NO || id;
        res = await dispatch(deleteLeaveEncashment(ref)).unwrap();
      }
      dispatch(fetchLeaveEncashments(currentStatus));
      return res;
    },
  ...
  return (
    <div className="space-y-4">
      ...
      description="Manage leave encashment requests"
      idPrefix="LER"
      domain="leave-encashment-request"
      fields={fields}
      columns={columns}
      initialData={[]}
      ...
      onBeforeEdit={async (item) => {
        ...
        const res = await fetch(`${API_URL}/leave-encashment-request/${encodeURIComponent(ref)}`);
