...
import { fetchWeekDays, addWeekDay, updateWeekDay, deleteWeekDay, clearWeekDayError, type WeekDayMasterGridData } from "@/lib/weekDayMasterSlice";
...
export default function WeekDayMasterPage() {
  const dispatch = useAppDispatch();
  const { weekDays, loading, error } = useAppSelector((s) => s.weekDayMaster);
  ...
  useEffect(() => {
    dispatch(fetchWeekDays(statusFilter));
  }, [dispatch, statusFilter]);

  useEffect(() => {
    if (error) {
      toast({ variant: 'destructive', title: 'Error', description: error, duration: DEFAULT_TOAST_DURATION });
      dispatch(clearWeekDayError());
    }
  }, [error, dispatch, toast]);

  const filtered = useMemo(() => {
    if (!Array.isArray(weekDays)) return [];
    return weekDays.filter((d: any) => {
      const searchable = [d.WEEK_DAY_NAME, d.REMARKS].join(" ").toLowerCase();
      return searchable.includes(search.toLowerCase());
    }).sort((a: any, b: any) => Number(b.id || b.WEEK_DAY_ID || 0) - Number(a.id || a.WEEK_DAY_ID || 0));
  }, [weekDays, search]);
  ...
  const openEdit = (item: any) => {
    setEditing(item);
    setForm({
      WEEK_DAY_NAME: item.WEEK_DAY_NAME || "",
  ...
  const handleSave = async () => {
    if (!form.WEEK_DAY_NAME?.trim()) {
      toast({ title: "Week day name is required", variant: "destructive", duration: DEFAULT_TOAST_DURATION });
      return;
    }
    ...
    try {
      const payload: Record<string, any> = {
        WEEK_DAY_NAME: form.WEEK_DAY_NAME.trim(),
      ...
      if (editing) {
        payload.WEEK_DAY_ID = Number(editing.id);
        const res = await dispatch(updateWeekDay(payload as WeekDayMasterGridData)).unwrap();
        toast({ title: res?.message ?? "Week day updated successfully", duration: DEFAULT_TOAST_DURATION });
      } else {
        const res = await dispatch(addWeekDay(payload as WeekDayMasterGridData)).unwrap();
        toast({ title: res?.message ?? "Week day created successfully", duration: DEFAULT_TOAST_DURATION });
      }
      setDialogOpen(false);
      dispatch(fetchWeekDays(statusFilter));
    } catch (e: any) {
      toast({ title: typeof e === 'string' ? e : (e?.payload || e?.message || "Error saving week day"), variant: "destructive", duration: DEFAULT_TOAST_DURATION });
    }
  };
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      const res = await dispatch(deleteWeekDay(deleteId)).unwrap();
      setDeleteId(null);
      toast({ title: res?.message ?? "Week day deleted successfully", duration: DEFAULT_TOAST_DURATION });
      dispatch(fetchWeekDays(statusFilter));
    } catch (e: any) {
      toast({ title: typeof e === 'string' ? e : (e?.payload || e?.message || "Error deleting week day"), variant: "destructive", duration: DEFAULT_TOAST_DURATION });
    }
  };
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Week Day Master</h1>
          <p className="text-sm text-muted-foreground">Manage week day master data</p>
        </div>
        <Button onClick={openAdd} className="bg-primary text-primary-foreground hover:bg-primary/90">
          <Plus className="w-4 h-4 mr-2" /> Add Week Day
        </Button>
      </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
          <div className="flex items-center gap-2 w-full sm:max-w-md">
            <div className="relative flex-1">
              ...
              <Input placeholder="Search week days..." value={search} onChange={(e) => { setSearch(e.target.value); setCurrentPage(1); }} className="pl-9 h-9 text-xs" />
            </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b bg-muted/50">
                  ...
                  <th className="text-left p-3 font-semibold text-muted-foreground uppercase text-xs">Week Day Name</th>
              ...
              <tbody>
                {paginated.map((item: any, idx: number) => (
                  <tr key={item.id || idx} className="border-b hover:bg-muted/30 transition-colors">
                    ...
                    <td className="p-3 font-medium">{item.WEEK_DAY_ID}</td>
                    <td className="p-3 font-medium">{item.WEEK_DAY_NAME}</td>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editing ? "Edit Week Day" : "Add Week Day"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-6">
            <div className="grid grid-cols-2 gap-4">
              <div className="col-span-2">
                <Label className="text-xs">Week Day Name <span className="text-destructive">*</span></Label>
                <Input value={form.WEEK_DAY_NAME || ""} onChange={(e) => handleFormChange("WEEK_DAY_NAME", e.target.value)} placeholder="e.g., Sunday" className={`h-9 text-xs${!form.WEEK_DAY_NAME?.trim() ? " border-destructive ring-1 ring-destructive/30" : ""}`} />
              </div>
              <div className="col-span-2">
      ...
      <AlertDialog open={!!deleteId} onOpenChange={(open) => !open && setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>This will permanently delete this week day record.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
