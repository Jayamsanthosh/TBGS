...
import { fetchOverTimeReferenceEntries, addOverTimeReferenceEntries, updateOverTimeReferenceEntries, deleteOverTimeReferenceEntries, submitOverTimeReferenceEntries, clearOverTimeReferenceEntriesError, type OverTimeReferenceEntriesGridData } from "@/lib/overtimeReferenceEntriesSlice";
...
export default function OverTimeReferenceEntriesPage() {
  ...
  useEffect(() => {
    dispatch(fetchOverTimeReferenceEntries({ status: statusFilter, fromDate, toDate }));
  ...
  const handleSubmit = useCallback(async (item: any) => {
    const ref = item?.OT_REF_NO;
    if (!ref) {
      toast({ variant: 'destructive', title: 'Error', description: 'Missing reference number', duration: DEFAULT_TOAST_DURATION });
      return;
    }
    setSubmittingId(item.id ?? item.SNO);
    try {
      const res = await dispatch(submitOverTimeReferenceEntries(ref)).unwrap();
      toast({ title: res?.message ?? "Over time reference entry submitted successfully!", duration: DEFAULT_TOAST_DURATION });
      dispatch(fetchOverTimeReferenceEntries({ status: statusFilter, fromDate, toDate }));
    } catch (e: any) {
      const msg = typeof e === "string" ? e : (e?.message || "Failed to submit over time reference entry");
  ...
  return (
    <div>
      ...
      <Dialog open={!!viewItem} onOpenChange={(open) => !open && setViewItem(null)}>
        <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>View Overtime Reference Entry</DialogTitle>
          </DialogHeader>
          {viewItem && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                {[
                  { label: "Ref No", value: viewItem.OT_REF_NO },
                  ...
                  { label: "ACC OT Ref No", value: viewItem.ACC_OT_REF_NO },
