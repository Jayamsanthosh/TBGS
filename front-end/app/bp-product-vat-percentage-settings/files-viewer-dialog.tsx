...
export default function FilesViewerDialog({
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    fetch(`${API_URL}/bp-product-vat-percentage-settings-files/${Number(current.SNO) || ""}`)
      .then((res) => {
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] flex flex-col overflow-hidden">
        <DialogHeader className="shrink-0">
          <DialogTitle className="flex items-center gap-2">
            ...
            Files Viewer
            {settingLabel && <span className="text-xs font-normal text-muted-foreground">— {settingLabel}</span>}
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="flex-1 min-h-0 flex flex-col lg:flex-row gap-4 overflow-hidden">
          {listLoading ? (
            ...
            <div className="w-full flex flex-col items-center justify-center py-16 border-2 border-dashed border-slate-200/80 rounded-[16px] bg-slate-50/30">
              ...
              <p className="text-sm text-muted-foreground font-medium">No files attached to this setting</p>
            </div>
          ) : (
            <>
