...
import {
  ...
  clearOvertimeRequestError,
  OvertimeRequestGridData,
} from "@/lib/overtimeRequestSlice";
...
export default function OvertimeRequestPage() {
  ...
  const { items, loading, error } = useAppSelector((s) => s.overtimeRequest);
  ...
  const { data: userStoreMappings } = useApiQuery("ot-req-user-map", async () => {
  ...
  const { data: employees } = useApiQuery("ot-req-employees", async () => {
  ...
  const { data: companies } = useApiQuery("ot-req-companies", async () => {
  ...
  const { data: departments } = useApiQuery("ot-req-departments", async () => {
  ...
  const { data: designations } = useApiQuery("ot-req-designations", async () => {
  ...
  const { data: departmentGroups } = useApiQuery("ot-req-dept-groups", async () => {
  ...
  const { data: designationGroups } = useApiQuery("ot-req-desig-groups", async () => {
  ...
  const { data: camps } = useApiQuery("ot-req-camps", async () => {
  ...
  const { data: stores } = useApiQuery("ot-req-stores", async () => {
  ...
  const { data: employmentTypes } = useApiQuery("ot-req-employment-types", async () => {
  ...
  const { data: currencies } = useApiQuery("ot-req-currencies", async () => {
  ...
  const { data: paymentModes } = useApiQuery("ot-req-payment-modes", async () => {
  ...
  const { data: banks } = useApiQuery("ot-req-banks", async () => {
  ...
  const { data: deptDesigMappings } = useApiQuery("ot-req-dept-desig-map", async () => {
  ...
  const { data: campStoreMappings } = useApiQuery("ot-req-camp-store-map", async () => {
  ...
  const baseFields: MasterField[] = [
    ...
    { key: "OT_FROM_DATE", label: "OT From Date", type: "date", required: true, placeholder: "Select Date" },
    {
      key: "OT_TO_DATE",
      label: "OT To Date",
      type: "date",
      ...
      validate: (_value: any, form: any) => {
        if (form.OT_FROM_DATE && form.OT_TO_DATE && !validateDateRange(form.OT_FROM_DATE, form.OT_TO_DATE)) return "OT To Date must be on or after OT From Date";
        return undefined;
      },
    },
    { key: "OT_HOURS", label: "OT Hours", type: "number", required: true, placeholder: "0.00", validate: (value: any) => {
      if (value != null && value !== "" && !validatePositiveNumber(value)) return "OT Hours must be greater than 0";
      return undefined;
    } },
    ...
    { key: "PAYMENT_REF_NO", label: "Payment Ref No", type: "text", placeholder: "Enter payment reference number" },
    {
      key: "PAYMENT_MODE_ID",
      label: "Payment Mode",
      type: "select",
  ...
  const handleFieldChange = useCallback((key: string, value: any, setForm: any, form: Record<string, any>): boolean => {
    ...
    if (key === "REQUEST_AMOUNT") {
      const untouched = form.APPROVED_AMOUNT == null || form.APPROVED_AMOUNT === "" || String(form.APPROVED_AMOUNT) === String(form.REQUEST_AMOUNT);
      setForm({ ...form, REQUEST_AMOUNT: value, APPROVED_AMOUNT: untouched ? value : form.APPROVED_AMOUNT });
      return true;
    }
  ...
  useEffect(() => {
    dispatch(fetchOvertimeRequests(currentStatus));
  }, [dispatch, currentStatus]);

  useEffect(() => {
    if (error) {
      toast({ variant: "destructive", title: "Error", description: error });
      dispatch(clearOvertimeRequestError());
    }
  ...
  const storeOverrides = useMemo(() => ({
    data: enrichedData,
    isLoading: loading,
    add: async (item: OvertimeRequestGridData) => {
      ...
      const payload: Record<string, any> = {
        ...
        OT_REQUEST_REF_NO: item.OT_REQUEST_REF_NO || `OT/${new Date().toISOString().slice(0, 10).replace(/-/g, "")}/${Date.now()}`,
      ...
      const res = await dispatch(addOvertimeRequest(payload as OvertimeRequestGridData)).unwrap();
      dispatch(fetchOvertimeRequests(currentStatus));
      return res;
    },
    update: async (item: OvertimeRequestGridData) => {
      const res = await dispatch(updateOvertimeRequest(item)).unwrap();
      dispatch(fetchOvertimeRequests(currentStatus));
      return res;
    },
    remove: async (id: string) => {
      ...
      const res = await dispatch(deleteOvertimeRequest(ref)).unwrap();
      dispatch(fetchOvertimeRequests(currentStatus));
      return res;
    },
    bulkRemove: async (ids: string[]) => {
      ...
      for (const id of ids) {
        const row = (Array.isArray(enrichedData) ? enrichedData.find((r: any) => String(r.SNO) === String(id)) : undefined);
        const ref = row?.OT_REQUEST_REF_NO || id;
        res = await dispatch(deleteOvertimeRequest(ref)).unwrap();
      }
      dispatch(fetchOvertimeRequests(currentStatus));
      return res;
    },
  ...
  return (
    <div className="space-y-4">
      ...
      onBeforeEdit={async (item) => {
        ...
        const res = await fetch(`${API_URL}/overtime-request/${encodeURIComponent(ref)}`);
