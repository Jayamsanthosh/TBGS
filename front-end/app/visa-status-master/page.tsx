...
import { fetchVisaStatuses, addVisaStatus, updateVisaStatus, deleteVisaStatus, clearVisaStatusError, type VisaStatusGridData } from "@/lib/visaStatusMasterSlice";
...
export default function VisaStatusMasterPage() {
  const dispatch = useAppDispatch();
  const { visaStatuses, loading, error } = useAppSelector((s) => s.visaStatus);
  ...
  const filtered = useMemo(() => {
    if (!Array.isArray(visaStatuses)) return [];
    return visaStatuses.filter((d: any) => {
      const searchable = [d.VISA_STATUS_NAME, d.VISA_STATUS_DESCRIPTION, d.REMARKS, d.STATUS_MASTER].join(" ").toLowerCase();
      ...
      return true;
    }).sort((a: any, b: any) => Number(b.VISA_STATUS_ID) - Number(a.VISA_STATUS_ID));
  }, [visaStatuses, search, statusFilter]);
  ...
  useEffect(() => {
    dispatch(fetchVisaStatuses(statusFilter));
  ...
  const openEdit = (item: any) => {
    setEditing(item);
    setForm({
      VISA_STATUS_ID: item.VISA_STATUS_ID || 0,
      VISA_STATUS_NAME: item.VISA_STATUS_NAME || "",
      VISA_STATUS_DESCRIPTION: item.VISA_STATUS_DESCRIPTION || "",
  ...
  const handleSave = async () => {
    if (!form.VISA_STATUS_NAME?.trim()) {
      toast({ title: "Visa status name is required", variant: "destructive", duration: DEFAULT_TOAST_DURATION });
      return;
    }
    if (form.VISA_STATUS_NAME.trim().length > 50) {
      toast({ title: "Visa status name must be 50 characters or less", variant: "destructive", duration: DEFAULT_TOAST_DURATION });
      return;
    }
    if (form.VISA_STATUS_DESCRIPTION && form.VISA_STATUS_DESCRIPTION.trim().length > 50) {
    ...
    if (!editing && normalizeStatus(form.STATUS_MASTER) === "INACTIVE") {
      toast({ title: "New visa status cannot be set to Inactive", variant: "destructive", duration: DEFAULT_TOAST_DURATION });
      return;
    }

    try {
      const payload: Record<string, any> = {
        VISA_STATUS_NAME: form.VISA_STATUS_NAME.trim(),
        VISA_STATUS_DESCRIPTION: form.VISA_STATUS_DESCRIPTION?.trim() || null,
      ...
      if (editing) {
        payload.VISA_STATUS_ID = Number(editing.id);
        const res = await dispatch(updateVisaStatus(payload as VisaStatusGridData)).unwrap();
        toast({ title: res?.message ?? "Visa status updated successfully", duration: DEFAULT_TOAST_DURATION });
      } else {
        const res = await dispatch(addVisaStatus(payload as VisaStatusGridData)).unwrap();
        toast({ title: res?.message ?? "Visa status created successfully", duration: DEFAULT_TOAST_DURATION });
      }
      setDialogOpen(false);
      dispatch(fetchVisaStatuses(statusFilter));
    } catch (e: any) {
      toast({ title: typeof e === "string" ? e : (e?.message || "Error saving visa status"), variant: "destructive", duration: DEFAULT_TOAST_DURATION });
    }
  };
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      const res = await dispatch(deleteVisaStatus(deleteId)).unwrap();
      setDeleteId(null);
      toast({ title: res?.message ?? "Visa status deleted successfully", duration: DEFAULT_TOAST_DURATION });
      dispatch(fetchVisaStatuses(statusFilter));
    } catch (e: any) {
      toast({ title: typeof e === "string" ? e : (e?.message || "Error deleting visa status"), variant: "destructive", duration: DEFAULT_TOAST_DURATION });
    }
  };
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Visa Status Master</h1>
          <p className="text-sm text-muted-foreground">Manage visa status master data</p>
        </div>
        <Button onClick={openAdd} className="bg-primary text-primary-foreground hover:bg-primary/90">
          <Plus className="w-4 h-4 mr-2" /> Add Visa Status
        </Button>
      </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
          <div className="flex items-center gap-2 w-full sm:max-w-md">
            <div className="relative flex-1">
              ...
              <Input placeholder="Search visa statuses..." value={search} onChange={(e) => { setSearch(e.target.value); setCurrentPage(1); }} className="pl-9 h-9 text-xs" />
            </div>
              ...
              <tbody>
                {paginated.map((item: any, idx: number) => (
                  <tr key={item.id || idx} className="border-b hover:bg-muted/30 transition-colors">
                    ...
                    <td className="p-3 font-medium">{item.VISA_STATUS_ID}</td>
                    <td className="p-3">{item.VISA_STATUS_NAME}</td>
                    <td className="p-3">{item.VISA_STATUS_DESCRIPTION || "-"}</td>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editing ? "Edit Visa Status" : "Add Visa Status"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-6">
            <div className="grid grid-cols-2 gap-4">
              {renderField("VISA_STATUS_NAME", "Visa Status Name", "text", undefined, true, "e.g., Approved", 50)}
              {renderField("VISA_STATUS_DESCRIPTION", "Description", "text", undefined, false, "e.g., Visa approved", 50)}
      ...
      <AlertDialog open={!!deleteId} onOpenChange={(open) => !open && setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>This will permanently delete this visa status.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
