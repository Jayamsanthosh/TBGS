                  ...
                  <div key={row.key} className="border rounded-lg p-3 space-y-3">
                    <div className="flex flex-wrap gap-3 items-start">
                      ...
                      <div className="flex flex-col gap-1 w-24">
                        <Label className="text-[11px] font-medium text-muted-foreground">Old Qty</Label>
                        <Input type="number" min="0" value={row.OLD_APPROVED_QTY} onChange={(e) => updateDtl(row.key, "OLD_APPROVED_QTY", clampNonNegative(e.target.value))} className="h-8 text-xs" />
                      </div>
                      <div className="flex flex-col gap-1 w-24">
                        <Label className="text-[11px] font-medium text-muted-foreground">Add/Remove</Label>
                        <Input type="number" min="0" value={row.ADD_REMOVE_QTY} onChange={(e) => updateDtl(row.key, "ADD_REMOVE_QTY", clampNonNegative(e.target.value))} className="h-8 text-xs" />
                      </div>
                      <div className="flex flex-col gap-1 w-24">
                        <Label className="text-[11px] font-medium text-muted-foreground">New Qty</Label>
                        <Input type="number" value={row.NEW_APPROVED_QTY} readOnly className="h-8 text-xs bg-muted/50" />
                      </div>
